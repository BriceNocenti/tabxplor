...
# col_vars (h vs k). This is the motivating fixture for the future `ref_n` field: with
...
golden_cases <- function() {
  ...
  list(
    ...
    # --- tab_many() multi col_var + weighting + ref_n motivating cases ---
    m_multi          = function() tab_many(syn, g, c(h, k), pct = "row"),
    refn_keep        = function() tab_many(syn, g, c(h, k), pct = "col", na = "keep"),
    refn_drop        = function() tab_many(syn, g, c(h, k), pct = "col", na = "drop"),
...
golden_display_cases <- c("f_row_pct", "f_ci_cell", "f_color_diff", "n_mean", "refn_drop")
