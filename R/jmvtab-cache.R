...
JMVTAB_CACHE_SCHEMA <- 9L   # bump on any store-shape change -> discard stale stores
...
jmv_tab3_build_armed <- function(data, opts, color, color_signif, ci, wt_sym,
  ...
  rlang::inject(tab(
    ...
    # Last Phase z14-i: `test` is a plain boolean. The jamovi test_robust selector offers only
    # classic / kish, and "kish" rides the global tabxplor.kish_neff option set in jmvtab.b.R .run()
    # -- so the rung is derived, exactly as in an R session. (The old "survey" branch here was dead:
    # `opts` never carried strata/ids, and the .a.yaml never offered that value.)
    test         = opts$chi2,
    na           = opts$na,
    levels       = opts$levels,
    ref          = opts$ref,
