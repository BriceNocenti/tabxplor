...
jmv_tab3_build_armed <- function(data, opts, color, color_signif, ci, wt_sym,
  ...
  rlang::inject(tab(
    ...
    # Last Phase z14-i / z16-iii: `test` is a plain boolean. The jamovi `design_effect` checkbox rides
    # the global tabxplor.design_effect option, set around the build in jmvtab.b.R .run() -- so the
    # inference BASIS is derived there exactly as in an R session, and the checkbox never has to be
    # translated into a second vocabulary here.
    test         = opts$chi2,
    na           = opts$na,
    levels       = opts$levels,
    ref          = opts$ref,
