...
jmv_tab3_build_armed <- function(data, opts, color, color_signif, ci, wt_sym,
  ...
  rlang::inject(tab(
    ...
    # Last Phase j: "survey" robust mode -> test = "survey" (+ strata/ids); "kish" rides the global
    # tabxplor.kish_neff option set in jmvtab.b.R .run(). "classic"/NULL -> the plain boolean.
    test         = if (isTRUE(opts$chi2) && identical(opts$test_robust, "survey")) "survey" else opts$chi2,
    strata       = opts$strata,
    ids          = opts$ids,
    na           = opts$na,
    levels       = opts$levels,
