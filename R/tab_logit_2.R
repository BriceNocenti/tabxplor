...
# #'
# #' Generate a 2x2 grid of standard diagnostic plots for a linear model object.
# #' This is a convenience wrapper around `plot.lm()` that automatically arranges
# #' the four diagnostic plots (residuals vs fitted, Q-Q plot, scale-location,
# #' and residuals vs leverage) in a single figure.
# #'
# #' @param model A model object, typically the result of `lm()` or `glm()`.
# #'
# #' @return NULL. The function is called for its side effect of producing plots.
# #'
# #' @examples
# #' # Fit a linear model
...
# #' Format odds ratios from logistic regression models
# #'
# #' Transforms odds ratios from a logistic regression model into readable format
# #' with probabilities, marginal effects, and optional confidence intervals.
# #' Supports multiplicative transformations for continuous predictors.
# #'
# #' @param model A fitted logistic regression model (e.g., from `glm()`).
# #' @param multiplicator Named numeric vector. Applies multiplicative transformations
# #'   to specified predictors. Names must match parameter names in the model.
# #'   Example: `c("age" = 10)` multiplies the age effect by 10.
# #' @param n Integer. Sample size for probability calculations. When `n > 1`,
# #'   probabilities are multiplied by `n` to show expected counts.
# #' @param digits_with_n Integer. Number of decimal places when `n > 1`.
# #' @param ci Logical. If `TRUE`, includes confidence intervals for odds ratios
# #'   and derived statistics.
# #' @param conf.level Numeric. Confidence level for intervals (default: 0.95).
# #' @param format Logical. If `TRUE`, formats output for display (converts ORs < 1
# #'   to "1/OR" notation, adds significance stars).
# #'
# #' @return A tibble with columns:
# #'   - `parameter`: Variable names
# #'   - `Odds_ratio`: Exponentiated coefficients
# #'   - `Odds`: Absolute odds (OR × intercept OR)
# #'   - `prob`: Predicted probability
# #'   - `marginal_effect`: Change in probability vs. reference
# #'   - Additional CI columns if `ci = TRUE`
# #'
# #' @examples
...
# #' readable_OR(model)
# #'
# #' # With confidence intervals and multiplicator for mpg (effect per 10 units)
# #' readable_OR(model, multiplicator = c("mpg" = 10), ci = TRUE)
# #'
# #' @export
# readable_OR <- 
...
#       dplyr::rename(parameter = term, Odds_ratio = estimate, z = statistic, 
#              dplyr::any_of(c(OR_inf = "conf.low", OR_sup = "conf.high"))) |>
#       dplyr::mutate(parameter = dplyr::if_else(dplyr::row_number() == 1, "Intercept", parameter))
#     # broom.helpers::tidy_plus_plus(model, exponentiate = TRUE)
    
    
#     if (length(multiplicator) != 0) {
#       missing_names <-  names(multiplicator) |>
#         purrr::map_lgl(~ length(which(OR$parameter == .)) == 0) |>
#         purrr::set_names(names(multiplicator) )
...
#       ref_multiplicator    <- names(multiplicator) |> purrr::map(~ which(OR$parameter == .))
      
      
#       names(multiplicator) <- paste0(names(multiplicator), "_", multiplicator)
#       mult <- list(names(multiplicator), ref_multiplicator, multiplicator) |> purrr::transpose()
      
#       OR <- 
#         purrr::reduce(mult, .init = OR, 
#                ~ dplyr::bind_rows(..1, 
#                            dplyr::summarise(..1, 
#                                      parameter       = ..2[[1]], 
#                                      dplyr::across(dplyr::any_of(c("Odds_ratio", "OR_inf", "OR_sup")), 
#                                             ~ dplyr::nth(., ..2[[2]])^..2[[3]]),
#                                      std.error  = abs(dplyr::nth(std.error , ..2[[2]])*..2[[3]]), 
#                                      p.value    = dplyr::nth(p.value   , ..2[[2]]),
#                            ))
#         )
#     }
...
#     OR <- OR |>
#       dplyr::mutate(
#         signif  = dplyr::case_when(p.value < 0.01 ~ "***", 
#                             p.value < 0.05 ~ "** ", 
#                             p.value < 0.10 ~ "*  ", 
#                             TRUE           ~ "   " ),
#         Odds    = dplyr::if_else(dplyr::row_number() != 1, Odds_ratio * dplyr::first(Odds_ratio), Odds_ratio), 
#         prob    = if(n <= 1) {
...
#         marginal_effect = set_digits( set_num(prob, get_num(prob) - get_num(dplyr::first(prob)) ), 
#                                       dplyr::if_else(n <= 1, 1L, digits_with_n) ), 
#       )
...
#       OR <- OR |>
#         dplyr::mutate(
#           Odds_inf = dplyr::if_else(dplyr::row_number() != 1, OR_inf * dplyr::first(OR_inf), OR_inf),
#           Odds_sup = dplyr::if_else(dplyr::row_number() != 1, OR_sup * dplyr::first(OR_sup), OR_sup),
#           p_inf    = if (n <= 1) {
...
#           me_inf   = set_digits(set_num(p_inf, get_num(p_inf) - get_num(dplyr::first(p_inf)) ),
#                                 dplyr::if_else(n <= 1, 1L, digits_with_n) ), 
#           me_sup   = set_digits(set_num(p_sup, get_num(p_sup) - get_num(dplyr::first(p_sup)) ),
#                                 dplyr::if_else(n <= 1, 1L, digits_with_n) ) 
#         )
#     }
...
#       OR <- OR |>
#         dplyr::mutate(
#           dplyr::across(dplyr::any_of(c("Odds_ratio", "OR_inf", "OR_sup", "Odds", "Odds_inf", "Odds_sup")),
#                  ~ dplyr::if_else(condition = . < 1, 
#                            true      = paste0("1/", format(1/., digits = 3)), 
#                            false     = paste0("   ", format(., digits = 2)))   ), 
          
#           dplyr::across(dplyr::any_of(c("prob", "marginal_effect", "p_inf", "p_sup", "me_inf", "me_sup")), 
#                  ~ stringr::str_pad(format(.), max(stringr::str_length(format(.))))
#           ), 
          
#           dplyr::across(dplyr::any_of(c("Odds_ratio", "marginal_effect")), ~ paste0(., signif)),
          
#           dplyr::across(dplyr::any_of(c("marginal_effect", "me_inf", "me_sup")), 
#                  ~ dplyr::if_else(dplyr::row_number() == 1, 
#                            true  = paste0("Ref:", rlang::eval_tidy(rlang::sym(prob_column[dplyr::cur_column()] ))) |> 
#                              stringr::str_remove_all("\\*") |> stringr::str_squish(), 
#                            false = .)),
          
#           p.value = format(round(p.value, 3), digits = 3)
#         ) |>
#         dplyr::select(-dplyr::any_of(c("signif", "p.value")))
#     }
    
#     if (n > 1) {
#       OR <- OR |> dplyr::rename("prob × n" = "prob")
#     }
...
#         index = dplyr::row_number() + 1L, 
#         dplyr::across(c(pct, `Empirical OR`, `OR model`, n), get_num)
#       ) |>
...
#           label  = dplyr::if_else(dplyr::lag(label, default = "") == label, 
#                                   "", 
#                                   label
#           ),  
...
# #' @keywords internal
# svglm2 <- function(formula, family = stats::gaussian(), data, weights, ...) {
#   data    <- tibble::add_column(data, wt = weights)
...
#     defaults = list(family = rlang::expr(stats::quasibinomial))
#   )
# )
