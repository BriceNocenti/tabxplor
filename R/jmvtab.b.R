  ...
  private = list(
    .run = function() {
      ...
      if (length(row_vars) > 0 | length(col_vars) > 0) {
        ...
        if (self$options$doExportExcel) {
          message("Before tab_xl()")
          tab_xl(tabs, path = self$options$xl_path,
                 sheets = "unique", open = FALSE, replace = TRUE)
          #tabs_hmtl <- ""
          # # reset the logical so it doesn't rerun next time (optional)
          # self$options$doExportExcel <- FALSE
          message("After tab_xl()")
