...
TAB_OPTIONS <- list(
  ...
  spark = tx_opt(
    ...
    c("in a [tab_reg()] table, a continuous predictor's `n` cell --- empty, since a continuous",
      ...
      "(the outcome binned against the predictor, on the model's scale, with no model in it)",
      "--- the eye-half of the `Linearity` footer row. With `tab_vars`, one curve per group.",
      "It is drawn TO SCALE on the predictor as the model sees it, so a `shape` transform",
      "visibly straightens it when it is the right cure, and every predictor's curve is the",
      "same width.",
