---
title: "Données pondérées et plans de sondage"
description: >
  Pondération, plans de sondage complexes, et les trois niveaux de marge d'erreur — celui que mérite un fichier donné, des tableaux croisés aux régressions.
...
Ce document sert à choisir entre les trois. Les tableaux croisés eux-mêmes sont dans [Introduction à tabxplor](tabxplor-fr.html), et le cadre de la régression dans [Tableaux de régression](tabxplor-reg-fr.html).
...
- [Introduction à tabxplor](tabxplor-fr.html) — les tableaux croisés, les couleurs et les intervalles de confiance.
- [Tableaux de régression](tabxplor-reg-fr.html) — le cadre de la régression.
- [Interpréter un modèle de régression](tabxplor-reading-a-regression-fr.html) — une analyse suivie du premier tableau croisé à une phrase finie.
- `?tab` pour `wt`, `design_effect` et `ci_method` ; `?tabxplor-options` pour les réglages de session.
- `survey::svydesign()` pour les plans complexes (strates, grappes, calage sur marges, corrections de population finie).
