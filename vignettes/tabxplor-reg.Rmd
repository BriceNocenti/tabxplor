...
The `empirical = TRUE` crude companion columns (the unadjusted `Obs_*` effects) are *descriptive*, so on weighted data their confidence intervals honour `options(tabxplor.kish_neff = TRUE)` exactly like `tab()`. The model columns themselves stay design-based regardless.
...
`split_var =` is the regression analogue of `tab()`'s `tab_vars`: the same model is fitted **within each level** of a grouping variable, and the per-group tables are stacked into one grouped table. It answers "does this effect hold in every subgroup?". Use `tab_spread()` on the split variable to lay the groups out as side-by-side columns for an easy comparison (a level absent from a group shows empty cells):
...
In a flat export the groups stay stacked, sharing one merged, vertical first column that names the split variable.
...
**`at = "reference"`** evaluates the effect at the reference profile instead of averaging it over the sample — a marginal effect *at the reference* for `effect = "ame"`, or a "category vs. the rest" odds ratio at that profile for a multinomial model:
