...
people: at `at = "reference"`, or under `na = "drop_by_model"` when a compared model drops different
...
|---|---|---|
| `effect = "ame"` / `"ame_ratio"`, any family | yes | **yes** |
| `family = "poisson"` on a binary outcome (risk ratios) | yes | **yes** |
...
| a *weighted* 3+ level outcome with `effect = "ame"` | yes | no (`marginaleffects` has no method) |
