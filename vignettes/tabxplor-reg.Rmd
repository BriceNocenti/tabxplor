...
With `trials =`, the observed columns are the average share of "yes" answers expressed two ways: `Obs_mean` is the plain **mean score**, and `Obs_OR` the odds ratio of the summed items.
...
|---|---|---|
| `effect = "marginal"`, any family and any measure | yes | **yes** |
...
**`effect = "at_reference"`** evaluates the effect at the reference profile (the abstract population combined the reference level of all predictors) instead of averaging it over the sample — a marginal effect *at the reference* for `effect = "marginal"`, or a "category vs. the rest" odds ratio at that profile for a multinomial model:
...
The same footer carries five **model checks**. You do not ask for them, and there is nothing to learn beyond the five nouns: each names an assumption, and the parenthesis names the instrument that measured it.
