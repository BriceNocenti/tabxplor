...
The raw percentages in `Obs_OR`'s brackets are the observed results everything in the model is derived from: "28% of black americans are married, compared to 51% for white americans"
...
- Here `Model_mRD` reads like this : "Comparing Black and White respondents who are alike in income, age and religion, being Black is associated with a marriage rate 19.8 points lower, on average."
...
- The modelised difference `Model_mRD` is very close, `-19.8%`, so very little of this difference between black and white can be explained by age, income or religion.

We can also compare the raw percentage — in the crude column's brackets — with the **model-adjusted probability**, in the model column's :
...
A 3+ level outcome would need one crude column per outcome category, so `empirical = TRUE` folds the crude effect **into** the model cell instead — `2.31 (2.05)` — and the crude percentages and differences appear in html tooltips, at mouse hover of a cell. Ask for the columns anyway with `empirical = "column"`.
...
|---|---|---|
| `effect = "marginal"` — binomial, multinomial, ordinal, Poisson | yes | **yes** |
...
**When do the three differ?** In a linear model, not at all: the coefficient, the average marginal effect and the effect at the reference profile are the same number (unless the predictor was fitted with a curve — see `shape =` below). That is exactly why `effect = "marginal"` is *refused* on a linear model, and on a Poisson rate ratio: the averaging changes nothing, and one quantity should not have two names. Ask for the coefficient. In a logistic, Poisson, multinomial or ordinal model the link is curved, so the effect of a predictor depends on where you stand on the curve. A coefficient then answers "for a person like this one", and an average marginal effect answers "for this population, as it actually is" — which is why an AME is usually the friendlier number to report, and a coefficient the more comparable one across studies.
...
Every cell below is the acronym the combination puts in the column header, after the constant `Model_` prefix (`diff` is `Model_diff`, `mRR` is `Model_mRR`). The first column says which outcomes reach each row, † marks the family's default measure, and ≡ marks a combination refused because it *is* the coefficient (see below).

| outcome kind      | `family`          | `measure`        | `"coefficient"` | `"marginal"` | `"at_reference"` |
|-------------------|-------------------|------------------|-----------------|--------------|------------------|
| numeric           | gaussian          | `"difference"` † | `diff`          | ≡            | `refdiff`        |
| numeric           | gaussian          | `"ratio"`        | `RoM`           | `mRoM`       | `refRoM`         |
| numeric           | gaussian          | `"odds_ratio"`   | not defined     | —            | —                |
| numeric (count)   | poisson           | `"ratio"` †      | `IRR`           | ≡            | `refIRR`         |
| numeric (count)   | poisson           | `"difference"`   | —               | `mdiff`      | `refdiff`        |
| numeric (count)   | poisson           | `"odds_ratio"`   | not defined     | —            | —                |
| numeric (score)   | binomial+`trials` | `"odds_ratio"` † | `OR`            | —            | —                |
| numeric (score)   | binomial+`trials` | `"ratio"`        | `RR`            | `mRR`        | `refRR`          |
| numeric (score)   | binomial+`trials` | `"difference"`   | `RD`            | `mRD`        | `refRD`          |
| factor, 2 levels  | binomial          | `"odds_ratio"` † | `OR`            | —            | —                |
| factor, 2 levels  | binomial          | `"ratio"`        | `RR`            | `mRR`        | `refRR`          |
| factor, 2 levels  | binomial          | `"difference"`   | `RD`            | `mRD`        | `refRD`          |
| factor, 3+ unord. | multinomial       | `"odds_ratio"` † | `OR`            | —            | `refOR` ¹        |
| factor, 3+ unord. | multinomial       | `"ratio"`        | —               | `mRR`        | `refRR`          |
| factor, 3+ unord. | multinomial       | `"difference"`   | —               | `mRD`        | `refRD`          |
| factor, ordered   | ordinal           | `"odds_ratio"` † | `cumOR`         | —            | —                |
| factor, ordered   | ordinal           | `"ratio"`        | —               | `mRR`        | `refRR`          |
| factor, ordered   | ordinal           | `"difference"`   | —               | `mRD`        | `refRD`          |

† the measure the family falls back on when you say nothing.
¹ each outcome category *versus the rest*, at the reference profile.
≡ the same number as the coefficient, so the call is refused and points at `effect = "coefficient"`.
...
- `≡` — **the coefficient itself**: the link is collapsible, so averaging the individual effects gives back the coefficient exactly. Two names for one quantity is what this vocabulary exists to avoid, so the call is refused and names the coefficient. It is why a linear model has no `effect = "marginal"` row: its coefficient already *is* the average marginal effect.

A further state exists only at run time: a link that does not converge on *your* data. The call says so, and for the risk difference it falls back to the linear probability model.

`family = "quasipoisson"` has exactly the grid of `poisson` — it changes the variance assumption, not what is estimated. And `measure = "log"` is not a fifth column: it is the family's default measure shown un-exponentiated, on the model's own link scale, and the header says which one it logs (`Model_log(OR)`, `Model_log(IRR)`, `Model_log(RoM)`, `Model_log(cumOR)`).

### What the header words mean

A header names the **measure**; the **contrast** is a marker on it. So there is one acronym to look up per quantity, and one marker to read on top of it.
...
The observed companion carries the measure **without** a marker (`Obs_RR` beside `Model_mRR`): it is a univariable effect, and where its levels come from the counts themselves a marginal and a conditional contrast are the same number.
...
- **`Model_RD` and `Model_mRD` are both percentage points, and are not the same number.** `Model_RD` is a conditional risk difference from an identity-link fit, `Model_mRD` a marginal one standardised over the logistic fit (see above) — which is what the `m` marker says.
...
- **One combination has no observed companion.** At `effect = "at_reference"` the model is conditional on one profile while the observed columns stay marginal over the whole sample, so the two are shown but not compared — `color = "adjustment"` and `{obs}` stay empty.
- **What needs an extra package, and what has no method.** `effect = "at_reference"` goes through **marginaleffects**; a *survey-weighted* multinomial or ordinal model has no marginal-effects method at all, so use the default `effect = "coefficient"` there. (`display` itself reaches every column, marginal or not.)
...
`effect = "marginal"` gives each group's average marginal effect. To get the cell table instead, cut
...
the same table both ways, `measure = "odds_ratio"` and `effect = "marginal", measure = "difference"`,
and say which one you are reading.
