...
Without a design, the observed columns' intervals already account for the weighting exactly (the flat survey design, `ids = ~1`) — a correction for unequal weighting only, blind to clustering and calibration. `tab_reg()` never reads `tabxplor.design_effect`: that option is `tab()`'s, and turning it on is what makes a `tab()` percentage interval comparable with these columns (see `vignette("tabxplor")`).
