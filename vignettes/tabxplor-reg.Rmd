...
It comes in **two blocks**. The first is one row per model you could fit: its `link`, and the measure
that model's own coefficients carry. The second is what is read off the model's *predictions* — the
same whichever link you fit, which is why those rows say `link = "(any)"`. So what `link` changes is
only which measure has a **coefficient**; every measure stays reportable from any of them.
