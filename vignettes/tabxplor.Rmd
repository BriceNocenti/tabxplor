...
### Multiple-answer questions: one column per item
...
The `tea` data (300 tea drinkers, from the `FactoMineR` package) has such a battery: *when do you drink tea?*, asked as six yes/no items. Everything below depends on one thing — the "yes" answer has to be each factor's **first** level:

```{r, echo = TRUE}
data("tea", package = "FactoMineR")
tea_when_vars <- c("breakfast", "tea.time", "evening", "lunch", "dinner", "always")

# Put the "yes" level first — it is the one the tables keep
tea <- tea |>
  tibble::as_tibble() |>
...
`display = "base_ci"` is the named layout for this: `{base}` is "the level this column shows" — a percentage on a factor column, a mean on a numeric one — so one call works for a mix of factors and numbers, and each column answers with its own quantity:
...
Colors are for exploring. For a journal, `theme = "print_minimalistic"` renders the same reading in black and white: **over-represented cells in bold**, *under-represented ones in italic*, an underline for the strongest threshold, and a grey fill for a second measure. The legend names the typography instead of the hues.
...
It draws **whatever the table computed** — no argument to learn: with `ci = "cell"` each cell's percentage with its interval, with `ci = "ref"` the difference from the reference with a zero line, with `display = "{or}"` the odds ratios on a log axis. `what = "level"` and `what = "effect"` override it.
...
* **It reads the table, and computes nothing.** It returns a plain `ggplot`, so `+ ggplot2::labs(...)`, `+ ggplot2::theme(...)` and `ggsave()` work; `theme = "print_minimalistic"` gives the greyscale version, `guide = "bands"` shades the panel behind the points with the colour scale itself (a good way to *show* a class what the colours mean).
...
- `ci_method = c(mean_diff = "student")` (the two groups pooled, a two-sample *t* test) or `"ols"` (pooled over **every** level of the variable — the interval a linear regression gives that coefficient) for a mean difference; 
- `ci_method = c(mean_ratio = "quasipoisson")` (scaled by the dispersion a quasi-Poisson regression estimates over every level — that regression's own interval) or `"poisson"` for a mean ratio. 
...
- **Every threshold is the same threshold, written in another measure.** The taught ±5 points is the first rung of `pct_diff`; read at a 50 % reference cell, those same 5 points are 0.1 SD, a ratio of ×1.1 and an odds ratio of ×1.2 — which is exactly where each of the other ladders starts. So a shade means the same size of deviation whichever measure you read a table on, and `set_color_breaks()` is where you move them all.
...
Because they compare to 0 and not to a reference, cell intervals are purely descriptive: they carry **no significance and no stars**, and *every* cell has one — the total row included. `ci_method = c(cell = )` chooses `"wilson"` (default), `"wald"` or `"beta"` (Korn-Graubard) for percentages; a mean cell interval is always the one-sample Student *t*.
...
- `display = "base_moe"` — print a confidence interval as a `pct ± margin of error` instead of the `[low; high]` bracket `display = "base_ci"` gives.
