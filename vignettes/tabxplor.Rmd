...
Under **unequal** weights that default carries no design effect, so the intervals run a little too narrow. One option corrects it:
...
| `wt = w` + `options(tabxplor.design_effect = TRUE)` | the unequal weighting, exactly — blind to clustering and calibration |
| a `survey` design as `data` | the full design: strata, clusters, `fpc`, calibration |

Turn the option on **if you want a `tab()` percentage interval to be comparable with the `Obs_*` column of a `tab_reg()` on the same data**: a regression table's crude companions are always corrected, beside a model column that always was, so at the default position the two functions disagree.
