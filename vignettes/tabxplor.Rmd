...
This is a simple single-stage approximation: it corrects for **unequal weighting only**, so it is blind to clustering and to calibration (and it needs the individual weights, so it is not available for tables built from pre-aggregated counts).
...
So there are **three rungs**, and it is worth knowing which one you are on:
...
**Check what your file can actually give you before reaching for rung 3.** Look for a stratum, cluster or calibration variable (`strate`, `grappe`, `psu`, `nomen`…). Many released research files ship one calibrated weight and nothing else — the design variables are withheld for confidentiality — and on such a file a design carries only the unequal weighting, i.e. it lands within a couple of percent of `kish_neff`. Rung 3 pays where the design information exists: a survey you ran yourself, a secure-access file, ESS/SHARE, the OECD assessments.
...
`tab_reg()` regression tables follow the same rule, models and observed companions alike (`vignette("tabxplor-reg")`).
