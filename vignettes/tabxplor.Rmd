...
Worth knowing before you reach for it: it pays off only where design information actually exists (look for a stratum or cluster variable in your file — if there is none, `kish_neff` is already all the correction available to you), and it costs roughly three times a weighted table. `tab_reg()` regression tables always use design-based standard errors for their **models** (`vignette("tabxplor-reg")`).
