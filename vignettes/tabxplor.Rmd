...
What the position shows is the **deviation** — the comparison your `color =` grades: a difference from the reference in percentage points, a ratio or an odds ratio on a log axis. The **level** it sits on is the number above the whisker, so the two say different things. Ask for `ci = "ref"` when you build the table, or there is no interval to draw. `what = "level"` swaps them.
...
* **Significance is the whisker.** It is coloured whole, so an interval that crosses the reference line is visible without stars — and there are none.
* **It reads the table, and computes nothing.** It returns a plain `ggplot`, so `+ ggplot2::labs(...)`, `+ ggplot2::theme(...)` and `ggsave()` work; `theme = "print_minimalistic"` gives the greyscale version, `guide = "bands"` shades the panel behind the whiskers with the colour scale itself (a good way to *show* a class what the colours mean).
