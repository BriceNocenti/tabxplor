...
Generic code cannot assume the shape of the table it is handed. Four accessors answer that, so a function can branch on facts rather than guess from column names.
...
`tab_supports()` answers **whether an operation applies**, before you try it:
