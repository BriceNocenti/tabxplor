...
Each field also has its own getter — `get_pct()`, `get_n()`, `get_diff()`, `get_mean()`, `get_or()`, `get_ci()` … :

```{r}
get_pct(tabs$Married)
```
...
Confidence intervals are stored as their two bounds; `get_ci()` returns the half-width, `get_ci_inf()` / `get_ci_sup()` the bounds:

```{r}
ci_tab <- tab(gss_simple, race, marital, pct = "row", ci = "cell")
get_ci_inf(ci_tab$Married)
get_ci_sup(ci_tab$Married)
```
