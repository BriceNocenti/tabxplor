...
* `ci` gains the value `"ratio"` (on `tab()` / `tab_many()` / `tab_ci()`): the confidence interval of
  the **ratio** between a cell and its reference (relative risk / mean ratio, the Katz interval),
  rather than of the difference. Independent of `color`.
