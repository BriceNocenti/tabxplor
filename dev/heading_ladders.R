# PURPOSE: the heading-ladder proposals -- the search record, and the ONE definition of every ladder.
# ROLE: sourced by the three dev previews (dev/heading_greens_preview.R writes the specimen page,
#   dev/site_theme_preview.R shows the settled ladder on the real site chrome, dev/palette_preview.R
#   borrows the colour maths). It writes nothing.
#
# ⚠ THE MATHS LIVES IN txtheme, NOT HERE. oklch_hex(), hex_oklch(), the gamut ceiling, APCA and WCAG
#   are txtheme's exported API -- one true source for the site theme and these previews alike -- and
#   the block below attaches them under the names the scripts already call. txtheme has no Imports,
#   so it is base R either way; installing it is `pak::pak("BriceNocenti/txtheme")`.
#   ⚠ dev-only: dev/ is .Rbuildignore'd and stripped from the release branch, so nothing in R/,
#     tests/, the tarball or a CRAN check depends on txtheme.
#
...
#
# ⚠ THE SETTLED LADDER IS `warm-95-10` (round ten), and it is the site's: txtheme's TX_PALETTE
#   records those six hexes and re-derives them from this very spec at load. Everything else here is
#   the search that reached it, kept so the choice can be re-read against its alternatives.

# === SECTION: the maths, attached from txtheme ====================================================
stopifnot(requireNamespace("txtheme", quietly = TRUE))
oklch_hex    <- txtheme::oklch_hex
hex_oklch    <- txtheme::hex_oklch
oklch_maxC   <- txtheme::max_chroma       # named max_chroma() there: what the SRGB ceiling is
oklch_cusp   <- txtheme::oklch_cusp
oklch_ramp   <- txtheme::oklch_ramp
oklch_ladder <- txtheme::oklch_ladder
in_gamut     <- txtheme::in_gamut
srgb_encode  <- txtheme::srgb_encode
srgb_linear  <- txtheme::srgb_linear
apca         <- txtheme::apca             # ⚠ use this, not contrast(), for anything dark
contrast     <- txtheme::contrast
