...
test_that("multinomial AME: one column per outcome category, matches marginaleffects", {
  ...
  for (j in c("Ind", "Dem", "Rep")) {
    ...
    expect_lt(max(abs(sort(get_pvalue(col)[keep]) - sort(acj$p.value))), 1e-5)   # absolute: see above
  }
})
...
test_that("reg ref = 'last' skips the NA level, and ref = 'NA' still names it", {
  d <- fx_reg_df()
  d$age[1:400] <- NA
  baseline <- function(rf, all = FALSE) {
    ...
    age <- as.character(r[[rv$var_col]]) == "age"
    as.character(r[[rv$row_var]])[if (all) age else age & is_refrow(fm)]
  }
  # named against the cut's own levels: the rule is "the last / first band", not a boundary value
  bands <- setdiff(baseline(c(age = "NA"), all = TRUE), "NA")
  expect_equal(baseline(c(age = "last")),  bands[[length(bands)]])
  expect_equal(baseline(c(age = "first")), bands[[1]])
...
test_that("grouped binomial (trials=) matches glm(cbind(s, q-s)); OR fmt shape", {
  ...
  # 1e-5: six significant figures prove the interval IS the quasi-binomial Wald one. Tighter would
  # test how far glm's IRLS happened to converge, which differs between two independent fits.
  expect_equal(sort(get_ci_inf(col)[keep]), sort(exp(unname(coq[, 1] - crit * coq[, 2]))), tolerance = 1e-5)
  expect_equal(sort(get_ci_sup(col)[keep]), sort(exp(unname(coq[, 1] + crit * coq[, 2]))), tolerance = 1e-5)
  # p-values are compared ABSOLUTELY: expect_equal()'s tolerance is relative, and a p of 3e-7 fails a
  # relative test against an independently converged fit while agreeing to nine decimals.
  expect_lt(max(abs(sort(get_pvalue(col)[keep]) - sort(unname(coq[, 4])))), 1e-5)
...
test_that("tab_reg() returns a grouped odds-ratio tab with the right structure", {
  ...
  expect_identical(get_display(col)[1], "or")     # the Constant: a baseline ODDS, not an effect
  expect_identical(get_display(col)[2], "est")
  expect_identical(get_pct_type(col), "row")
