# The parallel seam, moved WHOLE out of the shipped suite (Phase 23e).
# WHY: `tabxplor.parallel` is opt-in and FALSE by default, the file is skip_on_cran(), it needs
#   mirai (Suggests), and spawning the daemon pool costs ~10 s -- which set the shipped suite's
#   critical path on its own. "A parallel build is byte-identical to a serial one" is a property
#   to validate at release, not on every edit.
# ⚠ Re-run this after ANY change to R/tab-parallel.R or to a build stage it dispatches.

# PURPOSE: a test that cannot live in the shipped suite.
# ROLE: see dev/run_dev_tests.R for the five reasons a file is here -- source-tree lint, a
#   phase-defect suite, internal-seam parity, secondary engine parity, or an exhaustive sweep.
# USAGE: OMP_NUM_THREADS=1 Rscript dev/run_dev_tests.R

# === SECTION: the parallel seam ===================================================================
