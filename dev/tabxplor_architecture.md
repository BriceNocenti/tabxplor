...
it needs — `measure_stage()` (leaf vs test step), `measure_applies()` (can it colour a mean?),
...
for borders + a two-line span). See CLAUDE.md Phase 12c + Phase 18g + decisions §37. `R/tab_logit.R` and `R/tab_logit_2.R` are emptied (`git rm` pending; the
