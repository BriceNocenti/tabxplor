...
- color:
  ...
  the full grammar (`FALSE`/`TRUE`, a measure such as `"diff"`, a
...
- n_min:
  ...
  see [`tab`](https://bricenocenti.github.io/tabxplor/reference/tab.md)
  – that hides small-base rows/cells (largest base below `n_min` drops
  the row; own base below `n_min` blanks the cell) without recomputing
  anything.
