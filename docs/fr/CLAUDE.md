...
| Mean-diff asymmetry | For `type="mean"` columns, the `diff` field stores a **ratio** (cell_mean / ref_mean), NOT a difference. Thresholds like 1.15 mean “+15% above reference”. This asymmetry propagates into `color_formula()` and [`format.tabxplor_fmt()`](https://bricenocenti.github.io/tabxplor/fr/reference/format.tabxplor_fmt.md). **(1.4.0 §3: numeric `diff` becomes a real difference; the ratio moves to the `ratio` field — the never-used `rr` field renamed, placed after `diff`.)** |
