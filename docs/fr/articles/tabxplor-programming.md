...
but from those counts instead of the raw records; every calculation runs
on the counts, so the result is identical to what
[`tab()`](https://bricenocenti.github.io/tabxplor/fr/reference/tab.md)
...
- `tabxplor.plot_num_font` —
  [`tab_plot()`](https://bricenocenti.github.io/tabxplor/fr/reference/tab_plot.md)’s
