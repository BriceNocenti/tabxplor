...
- medium:
  ...
  media that draw the legend as coloured TEXT and cannot fill: an Excel
  rich-text cell
...
- classes:
  ...
  output (`tab_kable(engine = "html")`). Then the legend follows a theme
  toggle exactly like the cells it describes. `FALSE` (the kableExtra
  engine, which carries no stylesheet of ours) keeps inline hex.
