...
#### Phase 21g-xi — jamovi UIs French translations

French translations need to be very compact, because the size of the UI is very constrained, specially the width of the jamovi options panel. For `jmvtab`, reuse the vocabulary that was already translated in past versions. For `jmvtabreg`, nothing was reviewed by a human yet, so you can redo everything for consistency.
- The main vocabular is settled in `vignettes/articles/tabxplor-all-else-equal.Rmd` and `dev/french_glossary.md` which are cannon. If next common expressions and vocabulary must join this file, please edit it directly, then ask for the maintainer to review and, if needed, improve what you’ve done.
...
#### Phase 23f-i — all-else_equal vignette french translation

`vignettes/articles/tabxplor-all-else-equal.Rmd` is the most important vignette, because it carries  the most precise account of what tabxplor's *philosophy*, *vocabulary*, *usage* and *real-world regression use cases* really are; its words are the package's own. But this is also the most difficult to translate well.

 The goal is to find a really **idiosyncratic**, specific, consistent, readable, user-friendly, **stylistically strong and simple**, way of speaking about regressions in French – to teach them to "literary" social sciences students that hate math and don’t really understand english. For that, you should **avoid a bad word-by-word translation from english** entirely, too often done in the introduction and first regression vignettes: the aim is to **rephrase**, **rewrite another way**, do something a bit different for it to match the specificities of the French language and the specificities of French social sciences.
- `dev/french_glossary.md` should settle the twins for the new plain-language vocabulary (outcome / predictor / adjusted / "all else equal" / composition effect -> *effet de structure*); some research on it was done — Cibois's *effet propre*, Deauvieau's *« le langage du tableau croisé »*, INSEE's *à structure constante*. The maintainer is not even sure how to translate "to *fit* a model" in an accessible, readable French: the first thing to do is to expand the glossary. French translation here requires additional web searches across French quantitative social sciences literature and papers. You can modify `dev/french_glossary.md` directly, then ask me to review it and improve it manually.
- Using French should not make it more technical, neither should it make sound more familiar (speaking directly to the reader saying "tu" in french is too familiar and a bad idea; saying "vous" takes the risk of being too formal; that’s complicated !).
