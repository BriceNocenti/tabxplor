...
⚠ Measured starting point: interactions ALREADY FIT today through the formula escape hatch. But since this escape hatch lose most of tabxplor’s interesting feature, it would be really nice to remove it altogether before release. A separate internal `cross =` mechanism already builds the pooled `predictors * tab_vars` fit behind `color = "between_groups"` (`R/tab_reg.R:1966-1993`) — read it before inventing a second one.
