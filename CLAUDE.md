...
├── tab_reg.R       (~940 L)  Phase 12c/12d/12e-i: unified regression tables. tab_reg() over ONE engine
...
##### Phase 12e-ii – opt-in profile axis (deferred)

Add an opt-in profile axis (`at="reference"` / `newdata=`): MER-at-reference marginal effects + adjusted predictions at a fixed profile, which also delivers the 12d-deferred **"j vs rest OR at reference profile"** MNL flavour. Cheap with marginaleffects (`avg_slopes(newdata=datagrid(...))`).
...
- Make the color legend more easy to read for  non-expert users, and implement a French translation (detect OS language + override by optional argument in export functions ?) Here are meaningful exemples, in French (in every case, of course, it is only meaningful if each break is of the same color than in the table), that can be written via a script (knowing : ligne/colonne, reference Total or level name, and a string "Nuances de bleu", "Nuance du jaune au rouge" that can be baked with tabxplor default color palette, and have a fallback to saying which color it is with custom palettes ?) : 
  + Diff : "Nuances de bleu pour les cases >= à la ligne Total +5; +10 ; +20 ; +30 points. Nuances du jaune au rouge : <= à la ligne Total -5 ; -10 ; -20 ; -30 points."
  + Diff,  `color_signif="color_all_signif"` : "Nuances de bleu pour les cases >= à la ligne Total +5; +10 ; +20 ; +30 points. Nuances du jaune au rouge : <= à la ligne Total -5 ; -10 ; -20 ; -30 points."
En grisé : chiffre non significativement différent de celui de la ligne Total après prise en compte de la marge d’erreur (intervalle de Wald avec ajustement d’Agresti et Caffo, seuil de confiance à 95  %). 

  + Ratio : "Nuances de bleu pour les cases >= à : Total ×1,15 ; ×1,5  ; ×2 ; ×4. Nuances du jaune au rouge pour les cases <= à : Total÷1,15 ; ÷1,5 ; ÷2 ; ÷4."
  + OR, `color_signif="grey_non_signif"` : "Nuances de bleu : OR >= 1,15 ; 1,5 ; 2 ; 4. Nuances du jaune au rouge : OR <= 1/1,15 ; 1/1,5 ; 1/2 ; 1/4. Cases non significativement différentes de la modalité de référence (seuil de confiance 95%) passées en gris."
