...
⚠ **§8.3's `DISPLAY_TOKENS$label` already EXISTS** (Phase 22c-ii built it for the column names): a short name per token, a string or a `function(col)`, read through `display_token_label()` / `fmt_display_label()`. Reuse it for the tooltip line labels rather than declaring a second vocabulary — and note it is a name for a TOKEN, so §8.3's `role` qualifier (*adjusted* / *observed*) and the derived-geometry phrasing still have to be composed on top.
