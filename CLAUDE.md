...
- **Performance**: run the harness (see *Reference > Benchmarks*) before/after Phases 2, 3, 6; save to `dev/benchmarks/results_1.4.0/`; confirm the Phase 3 CI/chi2 win.
...
- `fmt()` public constructor (`fmt_class.R` ~L274): `refcol <- vec_recycle(vec_cast(totcol, ...))` casts `totcol` instead of `refcol` — the `refcol` argument is silently ignored (refcol always mirrors totcol). Low impact (refcol is normally set internally), but fix in Phase 1/2 (constructor + core rewrite).
