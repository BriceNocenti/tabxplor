...
**The maintainer's original items, for reference:**
...
#### Phase 22g-vi — yet another round of jamovi manual review

**⚠ Created by Phase 22g-v, and all one build-chain pass** (`dev/generate_jamovi_js.R` → `jmvtools::prepare()` → `i18nUpdate` → `install`):
- **`measure`'s value is renamed** `coefficient` → `raw_coefficient` in `jmvtabreg.a.yaml` (and `MEASURE_OF_RADIO` in the `.js`). Until then `dev/generate_jamovi_js.R` deliberately emits BOTH spellings into `TABX_ESTIMANDS` — the R alias resolves either, but a grid that named only the canonical one would grey a radio the panel can still set. Delete the `alias` line in the generator with the rename.
- **`digits` is missing from the Regressions panel**: `jmvtab`'s own 0-6 ComboBox, one pass-through (`tab_reg()` now takes it; named per-token values stay R-only).
- **`na` gained `keep_for_predictors`** — the picker does not offer it. It is the value a jamovi user wants most often ("what if I keep the non-response?").
- **`family` now offers `binomial` on a 3+ level outcome** (one level vs the rest); `TABX_OUTCOME_OFFERS` already says so, so the drop-down follows with no yaml change — worth a live check that `outcome_level` reads as "the level modelled" there.
- `stats` is gone from the panel (22g-iii) and `tab_reg(stats = )` now defaults to `"auto"`, so nothing to change; but the panel sends no `stats` at all, which is correct.

- In `jmvtabreg` reference picker, for numeric predictors, the `ref=` column shows "(model default)" but the default is just "mean" and it would be much more clear to say so to the user. Is "(model default)" needed at all in jamovi ?
- `jmvtab` Export button is broken (for Excel, but also md, etc.) with error "options@xl_check does not exist", because this particular one is `jmvtabreg` only.
- The off-pct-axis reference with `color="OR"`, which is column with `pct="row"`, still shows "ref = ..." in it’s third column, but should show "ref2 = ...". Also, it’s dropdown show a "Total" option, which is meaningless and should not be an option (the "100%" Total can’t be the reference for OR). Finally, in `tab()`, even for the pct-axis, it would be better if switching `color="OR"` could remove `ref="tot"` as a point of comparison, and silently fallback to `ref="first"` in this specific case (OR are not supposed to be versus the mean percentage of the column, but versus a specific category). `jmvtab` should do the same in js for the reference shown in the reference picker to be the right one.
- in `jmvtab`, make `other_if_less_than =` argument dormant: the merge and reorder factors UI can do the same thing manually, is more powerful, and the two are conflicting. Put `cleannames =` on the same layout row than `n_min =`. 
- strange thing happening in jmvtabreg with `theme = "print_ready"`: the html result shape table put some curves in the "outcome" column instead of the "observed shape one", for reasons I can’t understand. 
- `jmvtabreg` `outcome` × `family` × `link` × `outcome_level` table. 
  + For the options names of the `link =` column, remove the parenthesis. Only keep it for "auto (family based)" (rename that way). Droplist just a bit wider for the name to fit. Reorder the options names to follow the same order than measure : "auto (family based)", "difference", "ratio", "odds_ratio".
  + For the options names of the `family =` column, replace "multinomial (nominal)" with "multinomial" and "ordinal (ordered)" with "ordinal" (useless, wastes h-space).
  + When `cleannames=TRUE` (jamovi default), apply it to the outcome level picker options names too. 
- `jmvtab` and `jmvtabreg` references pickers and levels merges and ordering UI
  + The current text shows "x levels: click to relevel" when collapsed ; I finally also want it to show "x levels — click to close" when expanded. Also, I want the "click to ..." part to be in italics "#444444". And I don’t want a ":" but a " — ": "x levels – click to relevel"
  + When a levels reordering box is expanded, it’s first header row shows: "levels", "merge", "merge name". I want to remove this header row altogether, and write "<i>merge</i>" in the first row above the tick boxes (there’s nothing in there first row, by construction). The merge name text box have a default name written in it when it pops up, so it’s self-explaining. Also, if the whole level reordering and merging expanded box (including the up down arrows) could be indented a bit to the right (around 1cm?), to visually show it’s a children of the factor variable’s row, it would be great.
  + In the `shape =` column for numeric vars, I want to add precisions, and I want the options always ordered with numeric first and cut to factor second: "linear <i>(numeric)</i>", "log <i>(numeric)</i>", "sqrt <i>(numeric)</i>", "quadratic <i>(num.)</i>", "sd_bands <i>(cut)</i>", "median <i>(cut)</i>", "terciles <i>(cut)</i>", "quartiles <i>(cut)</i>", "quintiles <i>(cut)</i>", "deciles <i>(cut)</i>".
- **`shape =`**: rename `levels` to `values_to_levels` (don’ add "(cut)", it’s already too long) and move it last (it is the one that explodes the level count); put `sd_bands` above the quantiles (it is the row-axis default).
...
- **Cheaper paths ?**: a `jmvtabreg` reorder that does not change the reference still REFITS, where an `arrange()` on the cached table would do; and is there anything cheaper than a refit for a reference change, or for merged levels (estimate / CI)?
