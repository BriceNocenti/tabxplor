...
##### Phase 9b-3 — all `tabxplor_fmt` records ONCE at the very end

Implement the « materialize the `tabxplor_fmt` records ONCE at the very end » choice.
Assemble + level-drop + add_n + totals + the `tab_compact` merge run on the carrier; the single `new_fmt` moves to **after** `tab_compact`, **before** `tab_pvalue_lines`/`tab_apply_n_min`. **`tab_ci`/`tab_chi2` fold in here as plain-field writers** (9b-2's writers, subsumed — the carrier flows plain fields through ci/chi2). Landmines **L1–L7** land here. Recovers the leaf-build reconstruction (~30-48 % of the factor per-table build, §5) + the 0.16 s `vec_rbind` merge remainder.
