...
⚠ **Open, for the phases that own them:** `D` / `WR` must join **22c-v**'s unified alias table (they are in `REG_MEASURE_ALIASES` today, which the FK requires); `vignettes/tabxplor-reg-fr.Rmd`'s ordinal section is now stale, for **23f**.
