...
#### Phase 22g-ii — jamovi UIs manual review
...
##### `jmvtabreg` specifics
"Model" main panel
- the `family` + `link` + `trials` selector needs to be visually improved: the column names formatting (grey, small, plain fontface) is inconsistent with the other ones (black, same size, bold)
...
"Interactions" main panel
- Some features are broken: too much horizontal space taken, 
- It’s difficult to use. 

"Model comparison" main panel
- remove the "stats =" argument (the default is already good, only having "all" adds much noise, and one-by-one selection would be too costly ; R only).
...
#### Phase 22j — the fit digest: cache what a marginal effect needs, not the fit

**Goal.** Stop persisting raw model fits in the jamovi regression cache. Store instead a **slim per-fit digest** plus the **check scalars**, so that changing `measure` / `effect` re-runs g-computation from the digest and prints fast, while the store drops from MB to KB and the `$state` round-trip freeze goes away. Measured in Phase 22h; read its §8.5 and §7 before starting.

**The measurements this rests on** (Phase 22h, `dev/tabxplor_2.0.0_performance_review.md`):

- the jmvtabreg fit cache buys **×2.3** on a re-apply and **×1.1 on a changed option**, and costs **6.28 MB (1 model) to 15.89 MB (4 predictor lists)** serialised into jamovi's `$state` on EVERY UI round-trip — that is the freeze `jmvtabreg-cache.R`'s own comment describes;
- a full 21k-row binomial `glm` serialises at **2.417 MB**, of which `qr` 0.74 · `data` 0.65 · `model` 0.20 · `effects` 0.20 · `residuals` 0.10 · `fitted.values` 0.10 — every one of them length-`n`, and **none of them read by g-computation**;
- the slim digest (coef + vcov + `terms` + `family` + `xlevels` + `contrasts`) serialises at **0.032 MB — 76× smaller** — and `reg_gcomp_maker()` rebuilt on it returns `est`, `G`, `emp` and the delta-method SE **`identical()`** to the full-fit answer.

**Why it is well-shaped rather than open-ended: the extraction already exists.** `reg_gcomp_maker()` (`R/reg-influence.R:254`) reads exactly `delete.response(terms(fit))`, `family(fit)$linkinv` / `$mu.eta`, `coef(fit)` and its NA pattern — the DATA arrives as a separate argument, never from the fit. The categorical twin `reg_gcomp_cat_maker()` goes through `reg_prob_engine(fit)`, which is already a closure over `terms` + `coef` + the level set. So the work is to **hoist and persist what those two already extract**, not to invent an abstraction.

**The design.**

1. A `tabxplor_fitdigest` S3 class carrying, per family: `coef` (with its NA pattern), `vcov`, `delete.response(terms)`, `family` (or the link functions where there is none — ⚠ `polr` has no `family()`, and its `zeta` cut points are part of the parameter vector), `xlevels`, `contrasts`, the outcome `levels`, `nobs` / `df.residual`, and whatever `reg_prob_engine()` needs for `multinom` / `polr` / `svyolr`. Give it `coef` / `vcov` / `terms` / `family` methods so every existing caller works unchanged.
2. **The check scalars are computed EAGERLY, once, while the real fit is in hand**, and stored beside the digest: `dispersion`, `influence` (they share one sweep, `reg_check_influence_pass()`) and `collinearity` (`car::vif`). They are a property of the fit, do not depend on the reported measure, and so survive every `measure` / `effect` change — which is what makes dropping the fit statistically sound rather than a silent loss.
3. `at_reference` **loses its cache and that is accepted**: it is the one estimand whose engine is `marginaleffects` (`reg_marginal_engine()`), it needs the real fit, and it is the niche case. It must refit (or refuse) with a clear rule, never degrade to a wrong number.

**What must be audited — every consumer of `$fit`, classified digest / needs-the-fit.** Known needs-the-fit: `reg_check_influence_pass()`, `car::vif()`, the `linearity` refit, `brant`, `reg_check_plots()`, `broom::tidy()`, the `empirical` refits, and `marginaleffects`. Known digest-only: both g-computation makers, `reg_gcomp_baseline()`, `reg_delta_se()`. ⚠ `reg_check_rows()` already asks `reg_checks_for(has_fit = !is.null(f$fit))`, so a digest ALREADY drops the footer checks today — (2) above is what stops that being a regression.

**Traps, all measured or read in Phase 22h.**

- ⚠ `stats::model.matrix(tt, d)` on counterfactual data must reproduce the fit's own columns; the digest MUST carry `xlevels` and `contrasts`, and the existing `ncol(X) != length(b)` guard must stay — it is what turns a mismatch into `NULL` rather than a wrong number.
- ⚠ On an `svyglm`, `vcov(fit)` is ALREADY the design-based sandwich; the digest must store it as such and must not re-derive it.
- ⚠ `vcov(multinom)` costs **0.81 s** at 13 outcome categories (Phase 22h §6.2) — caching it in the digest is a large win there, and a second reason to do this.
- The digest must be **version-stamped** like the other cache tiers (`jmvtabreg-cache.R` already refuses a stale-shaped store), or a deserialised digest from an older build will be read with the wrong fields.

**Verification.** A parity matrix is the deliverable, not an afterthought: for every `(family × measure × effect)` that `reg_measures()` says is buildable, assert that the table built **from the digest** is `identical()` to the one built **from the fit** — the package's whole contract is byte-identity, and this phase moves the object every number comes from. Plus the store size before/after on the 1-model and 4-predictor-list cases (6.28 / 15.89 MB → expect KB), and a re-run of `dev/benchmarks/phase22h_parallel.R --child=jmv`.

**Ordering.** Independent of the decision to remove `stats` from the jamovi UI (see below) — but if that lands first, step (2) gets cheaper, because the check scalars would then only be needed for R-side users, who have the fit anyway. Do NOT let it block this phase.

**Also decided with it (own session, small):** remove `stats` from the jamovi UIs altogether — the default set is solid, and anything beyond it is expert territory that belongs in R.
