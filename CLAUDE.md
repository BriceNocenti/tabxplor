...
##### Phase 9b-3 — all `tabxplor_fmt` records ONCE at the very end (in progress)

Materialize the `tabxplor_fmt` record ONCE per row_var at the END of `tab_build_one` (BEFORE `tab_compact`, per the maintainer's decision — avoids landmines L3/L4, keeps compact/pvalue/n_min byte-identical by construction). The carrier flows leaf → col_var join → `tab_apply_tests` → `tab_assemble_tables`; `tab_ci`/`tab_chi2`/`tab_add_n_pct` become plain-field writers with the public `tab_ci()`/`tab_chi2()` kept as unwrap→shared-engine→rewrap wrappers.

**Done (2026-07-11): increment 1 — the single materialization seam.** `fmt_materialize_col()` (`R/tab.R`, the ONE `new_fmt()` call via `do.call`; `fmt_frame_fields`/`fmt_col_attrs` contract constants); both leaves route through it (byte-identical, perf-neutral, full suite green, no golden regen).
