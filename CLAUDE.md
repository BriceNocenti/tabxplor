...
├── jmvtab_reg-cache.R (~230 L) Phase 15b: the jmvtab_reg (Regressions) live-UI fit cache +
...
│                              change is a HIT) + jmvtab_reg_ref_vector (picker -> tab_reg reference)
├── jmvtab_reg.b.R   (~140 L)  Phase 15b: jmvtab_reg R6 backend (thin orchestrator, sibling of jmvtab.b.R;
│                              .h.R generated by prepare() -- inherit is lazy so it loads before then)
└── jmvtab_reg.h.R            Jamovi Regressions UI header (auto-generated by prepare(), NOT yet created)
```
...
— NO refit, byte-identical to a real refit (`test-jmvtab-reg-cache.R`). Heavy paths cache the raw fit
...
The single-model UI is built: 6 files (`jamovi/jmvtab_reg.{a,u,r}.yaml`, `jamovi/js/jmvtab_reg.js`,
`R/jmvtab_reg.b.R`, `R/jmvtab_reg-cache.R`) + `0000.yaml` registration + a `test-jmvtab-reg-cache.R`.
...
`R/jmvtab_reg.h.R` + the uijs blob, then `install(home='flatpak')` / `dev/build_jmo_windows.R`, then
live-review with `gss_simple`. Until `prepare()` runs, `check()` will NOTE `jmvtab_regBase` as an
undefined global (the .h.R that defines it is not generated yet) — expected, not a bug.

##### Phase 15b-ii (deferred): the model-comparison "+" builder

The "+" predictor-subset card builder (`predictors` = named list) + `compare`/`baseline`,
`multiplicator`, and `trials` widgets. The 15b-i fit cache already gives it "only fit new subsets".
