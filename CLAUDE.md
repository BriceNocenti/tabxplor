...
##### Phase 9b-5 — chi2 half DONE (2026-07-11); ci write-back = increment 2

**Done (byte-identical, full suite FAIL 0 | PASS 1354, NO golden regen; 10-shape git-stash `identical()` A/B):** two changes to `tab_chi2()` in `R/tab.R`. (1) The whole-table **TEST** on plain fields (`chi2_compute_test()`: mask + a fmt-FREE `select(!where(is_fmt))[mask2,]` group view + `get_n[mask]` marshalling, no `tabs2` fmt-slice) + the 9b-4 no-op removed. **Measured NO win** — the chi2 test isn't the cost (a 40×15 A/B was 0.1000 == 0.1000 s); the §6 "20%" was the DEFAULT-`calc` **contrib** writes, not the pipeline's `calc="p"` test. (2) The chi2 **CONTRIB writes** (`chi2_write_contrib()`): the per-cell `var`/`ctr` + `comp_all`/contrib-`color` moved from ~6 `mutate(across(where(is_fmt), set_*))` passes to **ONE** mutate over plain-precomputed vectors → **contrib per-table −41 % (1.7×), −30 % memory** (`dev/benchmarks/results_1.4.0/phase9b5_chi2.txt`); common/ci/numeric flat. Approach = **precompute-then-single-write** (real setters, NOT a `fmt_unwrap`/`fmt_wrap` round-trip). Two landmines: `var_contrib` is **per subtable** (old writes were grouped mutates → write runs ungrouped, then restores grouping); the old `dplyr::if_else()`-over-fmt **materialised `wn`** (NA→n), reproduced via `set_wn(get_wn())` under `"ctr"` calc. Dead `variances_by_group`/`cells_by_group` dropped.

**Increment 2 (next session) — the `tab_ci` carrier write-back (the +55 % lever):** `ci_inf`/`ci_sup`/`pvalue` + `ci_type`/`color`/`comp_all`/`display`, the grouped reference-row selection (**L6**, `last(which(is_totrow))` w/ `is_refrow` fallback), `detect_totcols`/`detect_refcol` name maps, the `tab_ci` L-IDX no-op-guard. `tab_ci` math already runs on plain vectors (`R/tab-agg.R` `ci_*`); only the orchestration is `dplyr`-over-fmt. Golden-locked by `test-golden`/`test-calculations`/`test-color-golden`.
