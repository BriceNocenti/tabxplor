...
#### Phase 22h — default-options-choice oriented full performance review

Do a **full performance review** based on **real world use cases** (see `dev/review_manual/xl_review.R`, `dev/review_manual/review.R`, `vignettes/articles/tabxplor-all-else-equal.Rmd`, and pick a selection that does not takes all day to run — max 10 minutes — to make reliable comparisons; also add the "huge dataframe" standard performance tests) on a reusable (.R ?) script. The aim is not mainly to improve performance (though you should report new or previously unseen performance bottlenecks or cheap ways to increase speed remaining), but to **balance the default options, so that speed stay acceptable, and costlier analysis or tests or features or the like are opt-in** (unless they are absolutery necessary for statistical soundness, etc., there may be justified exceptions). Write your results and findings in a clear, human readable and very detailed and structured new .md file, that would permit me to manually review what takes too much time and what is acceptable, and also indicates me the current state of performance in my desktop PC compared to tabxplor 2.0.0 dev starting 1.3.1 baseline (on the same data, to get a clear picture of the before/after overall speed gains).
- Do both one core and parallel tests – and if you find a reliable way to also simulate around what a "cheap 2019 French university All-In-One PC" would do it would be even better (that’s what my students will mainly use; make web searches if you need standard specs).
