...
### Phase 6b — Parallelisation research (done — 2026-07-09, NO-GO)

Researched whether parallelising `tab()`/`jmvtab()` over `row_vars` is a real perf win. **Verdict: NO-GO for 1.4.0 as a default — a bounded (~2–3.5×), regime-specific, opt-in-only gain at best; deferred.** Grounded PoC (mirai / base `parallel` / future.apply, W∈{1,2,4,12}, 8M + gss_cat): parallelising the pair axis is **byte-identical** (0/34 tables), but the speedup is **sub-linear and inverts the naïve prior** — the *small/typical* df is the sweet spot (3.5× on gss_cat, cheap setup) and the **8M df is the worst case** (memory-bandwidth-bound scan caps it ~2.5×, and 336 MB×W transfer makes a fresh call a net loss; few-tables always loses). data.table's own threading barely helps (`~1.2×`, bandwidth-bound); the real per-table cost is the **N-independent O(cells) fmt/chi2 overhead** — better attacked by the aggregate-core + Phase 7c cache (no dependency, no W×df memory blow-up). jmvtab *live* = no (cached aggregate → nothing to parallelise; caching is the lever). If ever built: Suggests-only opt-in `tab_pmap()` at the `tab_build()` seam, persistent workers + `setDTthreads(1)`, after Phase 2/7c. Full findings + tables: `dev/tabxplor_1.4.0_decisions.md` **§26**; scripts `dev/benchmarks/parallel_poc_{micro,tab,mirai_dispatcher}.R`, results in `results_1.4.0/phase6b_*.txt`.
...
- Use `dev/benchmarks` or create new ones. It should not reduce the performance of `tab()` with many variables in a significative way (one of `tab()` main use is a "create many exploratory tables with a tons of variables at once and export them for manual review with color helpers" workflow).
