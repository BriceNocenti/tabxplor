...
- **What the Constant row holds** — maintainer's decision: *do nothing, document only*. It is settled in **22b-vii** instead, where either centring by default or a user-declarable numeric reference (0 or mean) makes the row correct with no work of its own.
...
**The Constant row, documented rather than changed** (per the decision above). `?tab_reg` now says what the row is — the model's intercept, every factor at its reference level and every numeric at **zero**, which for an age or an income is outside the data — that this is NOT the reference profile the rest of the table uses (`at_reference` puts a numeric at its mean), and where it is empty and why (`marginal` / `at_reference` have no intercept to show, an ordinal fit has thresholds instead of one). The correction matters because the page promised the opposite. A `# DESIGN:` tag at `reg_column()`'s Constant exclusion points at 22b-vii. ⚠ **Reported, not changed**: the row's label is *Reference population*, which over-promises for the same reason — worth settling with the profile itself in 22b-vii.
...
- **`center` is the new knob and the sharpest test of the assessment, AND it now owns the Constant row.** 22b-iii deliberately left that row alone and only documented it, so this phase settles both at once: a numeric predictor's REFERENCE (0 or its mean, per variable) is what the intercept sits at, so declaring it here makes the *Reference population* row correct with no machinery of its own — and would let the row's label stop over-promising. 22b-v establishes that centring is a no-op on every displayed coefficient, that its one real effect is exactly that row, that the right centre is a per-variable fact — a count's meaningful anchor is 0, not its mean — which is exactly why it belongs in a per-variable spec if anywhere rather than in a scalar option, and that transforming the fit puts the crude column, computed on RAW data, into a different unit system. So `center` must either un-centre every displayed level consistently, on both sides of the crude/adjusted pair, or be judged not worthwhile. Read 22b-v first.
...
###### Part B - numeric predictors centering ?

**Should numeric predictors be CENTERED by default? This is the first draft of the research: you should push it forwards.** The four questions asked — good default? sound for counts as well as continuous? global option or vectorised argument? — answered from the code, a measurement and the literature.
- **Centring changes nothing tabxplor currently displays (no interactions), except the intercept.** It is an exact reparameterisation of the linear predictor, so it holds for every family and every link, not just for OLS. Measured on `married ~ race + rincome + relig + age` (logistic, `gss_cat`): every non-intercept coefficient and every standard error is identical to machine precision — max |Δcoef| `3.3e-16`, max |Δse| `2.8e-17`. As a default data transformation it would therefore buy nothing at all, and "robust or not" does not arise: it is arithmetic, not an estimator choice.
...
- **Centring is the most maintainable route (the fit still calculates everything).** If a coefficient table ever wants the Constant row's standard error and p straight from the fit rather than from a prediction, centring IS the cheaper route and Schielzeth's argument applies directly. Since the two numbers coincide, that is a maintainability preference, not a correctness one.
- **It would not be perfect for count predictors due to the distribution, neither would it be absurd.** The guidance converges on centring only where zero is absent from the data or meaningless, and on preferring a substantively meaningful anchor where one exists — and a count is precisely the case where zero is both meaningful and observed (`tvhours = 0`, `children = 0`). "At 0 hours of television" is a better baseline than "at 1.97 hours", which describes nobody and is not even an attainable value. Skewness is a red herring: the mean is not the best anchor for a count because it is not a real value of the variable. But at the same time, **it’s more meaningful to have the reference profile at the "mean number of tvhours", or children, than having it for "the individuals with age 0 years"**. (What genuinely does need care on a count is the per-SD CONTRAST, which the package already applies — an SD of 2.07 television hours is a defensible step, an SD of 0.5 children is not; `?tab_reg` already warns about the latter.)
...
- **The VIF argument is to be taken into account.** That comment records the pair's own VIF falling from 38.7 to 1.2, which is a property of the PARAMETRISATION, not of the fit, but makes the whole uncentered GVIF unreliable because its **false-alarming** on a curved term. 
...
**Some open questions remains:**
- If we center numeric predictors before the fit, **are there currently calculated and displayed quantities that would need to be un-centered after the fit** to stay meaningful for the user ?
- Would **extending the `ref` argument for numeric predictors** be a good, reliable and user-friendly idea, with the possibility to enter a value (`0`, `4`, `1.127`) or `"center"` (`"mean"` instead ? Which one is more standard, which one is more readable for literary social sciences students ?) or `min` etc. (what other options would be meaningful here, or are common in regression packages and apps ?) ?
...
⚠ Measured starting point: interactions ALREADY FIT today through the formula escape hatch. But since this escape hatch lose most of tabxplor’s interesting feature, it would be really nice to remove it altogether before 2.0.0 release. A separate internal `cross =` mechanism already builds the pooled `predictors * tab_vars` fit behind `color = "between_groups"` (`R/tab_reg.R:1966-1993`) — read it before inventing a second one.
