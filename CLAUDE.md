...
#### Phase 22g-vii — yet another round of exports manual review — DONE

**Every review item had one named cause, and in every one of them the fact the fix needed was already declared somewhere — it was simply not read at the point that decides.** So the phase is a set of *reads*: one width vector instead of four literals, one rotation decision instead of two, one display boundary instead of a bypass, one table identity that outlives its own attributes.

⚠ **The worst defect was found by running the maintainer's own snippet: `set_display(x, "difference")` printed the COUNT.** `display_resolve()`'s header claims to be "THE display boundary, shared by `tab(display =)`, `tab_reg(display =)` and `set_display()`" — and for anything that was not a preset name the setter wrote the string RAW into the `display` field, so the abort that exists (`validate_display_template()`) was never called and `format()` fell through to another field. The claim is true now: every public `set_display()` value is answered by the declared vocabulary, stored CANONICALLY (an alias becomes its token), and an unknown word aborts naming the valid fields. ⚠ The gate is on the PUBLIC setter only — `fmt_set_display()` stays the raw writer the pipeline's per-cell token writes need — and it accepts every declared token, not the `?tab` subset, because the pipeline sets `n_range` / `blank` / the footer tokens through the same function. Measured: `+12%` / `×1.16` / `2.11` where `955` / `×1.16` / `955` stood.

**And a measure's own name is now a legal display value, derived rather than listed.** `DISPLAY_TOKENS$comparison` already declares "the colour MEASURE this token names"; its inverse is the map, asserted single-valued at build. `"difference"` → `{diff}`, `"odds_ratio"` → `{or}`, `"ratio"` → `{ratio}` — so one word means one quantity whether it is typed in `color =` or in `display =`, which is the code half of the vocabulary rule Phase 23b queues for the prose. A word that is both a token and a measure ("ratio") is caught by the token test first and resolves identically.

**Excel column widths are measured, and `colwidth = "auto"` is the default.** They were four hard-coded literals — 30 for the row-label column, `colwidth = 10` for every numeric one, 3.5 for the name column, and `"auto"` delegating to openxlsx2's `bestFit` — re-derived a second time inside `xl_prose_span()`, whose own comment admitted it. There is now ONE vector (`xl_col_widths`), computed in the plan from what each cell will show: `format()` renders the very strings Excel displays (it is also where the numFmt codes come from), and the level headers and unit tags are already in the render model. **The rule the whole vector rests on: a column is as wide as the widest thing in it that CANNOT wrap, and everything that can contributes its width divided by the lines it may use** — a figure never wraps, so the data set the floor; the level header and the unit tag both wrap (the unit row's `wrap_text` was turned on for exactly that) and Excel fits their row's height itself. A text column is capped by **`wrap_rows`**, the one argument that already says how wide a row label may be, so there is no second number. ⚠ **Widths are set per SHEET, not per table**: a column index belongs to the sheet, and with several tables stacked the last writer silently overwrote the earlier ones — reduced with `pmax` now. `bestFit` is deleted rather than wrapped: it blanks merged ranges before measuring, ignores `wrapText`, rotation and the per-cell font, and runs at the end of each table. Measured on the maintainer's own table: `30 | 10 × 6` becomes `6.7 | 8.3 | 8.3 | 9.4 | 10.5 | 9.4 | 7.2`.

**Excel joins the wrap system** (`wrap_rows` / `wrap_cols` gain `tab_xl` in `EXPORT_ARGS`): it passed no wrap spec at all, so a long label reached the sheet whole and only Excel's own mid-word break could hold it. It now breaks at the same points html does, with `brk = "\n"` and ordinary spaces — the U+202F substitution exists to stop a BROWSER re-breaking what we already wrapped, and Excel does not. ⚠ Two merged cells then needed an explicit height, because **Excel never auto-fits a merged cell**: the col_var span row and any merged label run, both from one `xl_row_lines()` estimator.

**A variable name rotates only when the rotation saves width.** The old rule was `run$span > 1` — blind to the name, made twice (html and Excel), and it left "Constant" and every numeric predictor horizontal while turning their neighbours. It is one prep fact now (`tab_vname_plan` → `roles$vname_plans`), read as the `tx-vname` class by html and as a 90-degree rotation plus that column's width by Excel:

- a ONE-ROW block cannot turn (a rotated single cell only makes its row tall);
- **those forced-horizontal names set the FLOOR**, because the column is already as wide as the longest of them — so `employed` beside `Constant` stays horizontal although it is narrower, which is the maintainer's own case and the point of the rule;
- a name turns only if it is longer than that floor AND its block is tall enough (`TX_VERT_CHARS_PER_ROW = 2` rotated characters per row, at most `TX_VERT_MAX_LINES = 2` lines);
- the floor is capped at `TX_VNAME_MAX` (13, promoted out of `tab_xl.R` so both media share it) or at `wrap_rows` when it is lower — a name column exists to name blocks, not to be read as prose.

⚠ The per-row width must be carried DOWN the run: a block's continuation rows hold the same name, and a width that differed there wrapped them differently — which changes the VALUE, and the label runs are read off the values, so one block split into two and the name printed twice. Found by tripping it.

**A rotated name wraps to its block's own height** — `span × TX_VERT_CHARS_PER_ROW` characters per vertical line — so there is no `wrap_rows_vertical`: the renderer knows the height and a user preference could not be better informed. That was the maintainer's decision, and it is what retires `html_break_cross()`, the hand-rolled `*`-only break helper whose comment said it existed because `tab_wrap_text()` could not do it.

**A NAME IS A COMPOUND WORD, NOT PROSE** (`tx_wrap_name`, beside `tx_str_wrap` in `utils.R`). `stri_wrap()` breaks on whitespace, so a `snake_case` variable name — the commonest kind in a data frame — was never wrapped at any width, on any medium, and no cap could hold it. The break opportunities are declared: **after** a space, `_` and `.` (the separator stays at the end of its line, where it reads as "continues below"); **before** `*` and a lowercase→uppercase camelCase seam. `-` and `/` are deliberately not opportunities — far more often a range ("25-34") than a seam. A run with no opportunity in it is hard-broken, so a cap is always honoured. Continuation lines carry the maintainer's one-character indent, written as U+00A0: the html path rewrites every remaining space into U+202F, and a plain-space indent would be caught by that blanket rule and shrink to a fifth of its width. The name column is re-wrapped by the prep at its own derived width, over `tx_unwrap_text()` — the helper that exists to undo the generic wrap.

**Two facts a downgraded table was losing, both now read off the columns.** (1) **The rules between `row_var` blocks** rode `dplyr::group_indices()`, which answers 1 for every row of a table that lost its `grouped_df` class — so a merged table put through `as_tibble()` lost every separator, silently, on all four backends. They come from `tab_label_runs()` now: it is already computed, already documented as reading the VALUES rather than the grouping, and its innermost column's run starts ARE the group combination. One definition, one more consumer, the grouping read gone. (2) **The table's kind**: `tab_kind()` already fell back spec → `test`; it gains a third, column-derived stage — a column whose `role` is `"model"` or `"emp"` makes the table a regression. That is a per-column `fmt` attribute, so it outlives everything dplyr drops, and it fixes every `tab_is_reg()` reader at once (the base-count column the maintainer noticed, the footer branch, the shape table, `print_ready`'s per-table palette choice) instead of patching one. The crosstab side of his `n` question was left alone by his own decision.

**A col_var is named once per header.** The Total column carries no label of its own, so a variable whose columns are split by it opened a SECOND labelled run and the span row printed `released` twice. Blanked, keyed on (label, sub-population) so a spread still names its variable once per group — the row axis's own rule applied along the column axis, beside `tab_col_units()`'s "written ONCE per col_var, in its leftmost column".

**A publication palette's marks are ink, not aside grey.** They fall outside the primary character range on purpose — they sit where the stars sit — so every backend painted them with `grey2` (#444444), too faint for a superscript glyph. `tx_chrome_hex()` gains **`mark`** (pure black under every print palette, `grey2` elsewhere, where nothing writes a mark), and `format()` records the run's character range as `mark_nchar` at the same two sites and by the same mechanism that already records the primary's. `fmt_cell_suffix()` — the one place stars and marks are told apart — flags which it returned, so no backend re-derives it from the theme. html paints `tx-mark`, the console its ANSI twin (`paint_split()` gained the fourth piece), and **Excel needed nothing**: the marks are folded into the numFmt literal, so they already take the cell's own ink. Markdown needed nothing either — there the slot class colours the whole cell, so the marks were never grey.

**Two defects the phase exposed rather than introduced, both found by tripping them.** ⚠ **The label columns were nested in the wrong order.** `tab_label_runs()` scans outer → inner and was handed `c(name_col, tab_vars)` — a fixed order, while a REGRESSION SPLIT BY `tab_vars` has both kinds of label column at once and the kept tab_var sits FIRST. So `race`'s run was cut at every predictor change: its level was repeated down the table instead of spanning its block, and (once the rotation rule read those spans) it could not turn either. `tab_label_order()` sorts them by their position in the table — the nesting IS the column order — and the "mutually exclusive by construction" note that missed this is corrected where it stands. ⚠ **And a spread's suffix strip had the `tx_unwrap_text()` blind spot for the third time**: `endsWith(clean, "_Ensemble")` is compared against the RAW `col_group`, so it stopped matching the moment a column name was long enough to wrap — and the header then labelled itself with the LEVEL instead of the variable. It reads through the un-wrapper now. ⚠ That un-wrapper needed one rule of its own: **a break `tx_wrap_name()` put at a seam is removed, not turned into a space** — it broke a compound name at a separator the name already carries, so inventing a space there is exactly what would keep the comparison failing.

**Tests.** New locks, kept to what the phase earns: `set_display()` on a measure name, on an alias and on a nonsense word (and that a field ALREADY holding a malformed template still renders — the old test injected one through the public setter, which now refuses it, so it writes through the raw one) · a col_var named once in the span row · the rotation rule's cases — `Constant` forcing `employed` horizontal, a 43-character `snake_case` name wrapped within the cap · `tab_kind()` from the columns alone, and a stripped merged table keeping its block rules · `.tx-mark` black while `.tx-sec` stays `#444444`, and no mark span at all under a colour theme · Excel widths fitting their content and surviving a second table on the same sheet. Updated: the sd-column width test now says `colwidth = 10` where it meant it (and asserts that "auto" is strictly more compact), the name-column width tests state the new rule, and the interaction-break test picks a name long enough to rotate. Full suite **FAIL 0 | PASS 9674**; the ONLY golden diff in the phase is the one new `.tx-mark` CSS line, emitted in the print layer alone.

**Still open for the maintainer**: `theme = "print_marks"` on paper (the marks should now read as strongly as the numbers), an Excel export opened in Excel (the widths, the wrapped headers, a wrapped merged label), and whether `Model fit` rotating one character past the floor set by `Constant` reads well beside four horizontal names — the rule is literally his, and this is its one marginal case.


#### Phase 22g-viii — the interactions picker, folded into the model builder — DONE

**One control now holds both halves of "which predictors does this model have", and the change that mattered was not the layout.** `crossPickerCtrl` and its *Interactions* CollapseBox are deleted; `modelBuilderCtrl` renders the interaction rows above the model cards, in the per-outcome Model table's own material. The box is *Interactions, models comparison and predictor subsets*. Every jamovi fact that cost time is in `dev/tabxplor_2.0.0_jamovi_dev.md` § Phase 22g-viii.

⚠ **The old fold made a with/without comparison IMPOSSIBLE, and its comment said the opposite.** `jmvtab_reg_cross_fold()` replaced both parents by the key in **every** card that held them — so the moment `age*race` was defined, a card holding `age` and `race` *became* the interaction model and the additive one could not be asked for at all. That is the one comparison defining an interaction exists for. A pair is now **DEFINED once and TICKED per model**: the fold runs on the ZERO-CARD path only (no card → nowhere to tick → every defined pair applies to the single live model, stated in a hint because that replacement is otherwise invisible), and a card carries its own interactions. Measured on `gss_cat`, two cards over `race + age + relig` with the second ticking `race × relig`: the footer carries **`compare_seq` χ² = 36.42, df = 14, p = 9.0e-4**, exactly equal to that model's own `cross_lr` row — a real sequential LR, not a ΔAIC fallback, because `reg_cross_expand_terms()` expands the combined factor back to its parents and `reg_compare_chained()` then sees the nesting between two models whose term labels share no name.

⚠ **The roadmap's "a card's `vars` may simply hold `a*b`" does not work, and the reason is worth keeping.** `vars` is `type: Variable` and `jmvtab_reg_models()` does `intersect(pool, card$vars)` against the real column pool, so the key is dropped R-side. Retyping `vars` to `String` fixes that at the cost of the cards' **rename-safety** — jamovi rewrites a `Variable` option when a column is renamed, and a card silently losing a predictor is a wrong model. So a card carries a second list, **`crosses: Array of String`**; the nested `Array{Group{String, Array{Variable}, Array{String}}}` compiles cleanly (checked against the generated `.h.R`). Two sweeps keep it honest, both in `reconcileModels()`: a key no longer defined is dropped, and so is any **parent** of a key the card still holds — `reg_parse_crosses()` refuses that pair, and a picker must never write a combination the producer refuses. `jmvtab_reg_models()` mirrors the second one (`setdiff(v, jmvtab_reg_cross_parents(k))`), because the R boundary must not trust the UI either.

⚠ **`jmvtab_reg_staged()` needed the keys, and finding out cost nothing only because the shape was written down.** A card holding ONLY an interaction has an empty `vars`; without `cross_keys` it reads as an empty card, is dropped, and a two-model comparison would run **live** instead of behind the Run button — the freeze that gate exists to prevent. It takes `cross_keys` now, and `.b.R` hands it the same ones `.opts()` folds.

**`race*race` is unreachable rather than repaired** (the maintainer's item, and the rule that deleted `linkOffered()` in 22g-i). Both drop-downs offer the whole pool, and picking the variable already on the OTHER side **swaps the pair** — which is what the user meant, `a*b` being "the effect of a within each level of b", and it keeps the flip one click away. The old code stored the collision and then stepped the other side to `free[0]`: a silent change the user did not ask for, whose `<select>` was never repainted. Options are additionally filtered so no pick can duplicate a pair another row defines, and `+ Add interaction` seeds the first **free** ordered pair (two clicks used to make `pool[0] × pool[1]` twice, which R silently deduped — a row on screen contributing nothing) and greys out when every one is taken. `reconcileCrosses()` and `jmvtab_reg_cross_keys()` keep their own `var1 != var2` guards, because a pair stored by an older build must not reach R either.

**The layout, in the Model table's idiom.** An `mtCard` around the rows, an `mtHead`-style header naming what each column IS (`effect of` · `within each level of`), and `crossRow` on the same fixed-column grid — ⚠ the two repeat one `grid-template-columns`, the warning `mtRow`/`mtHead` already carry. The old `width:74%; min-width:360px` **plus** a horizontal margin is what pushed the delete button out of a narrow pane (22g-iv's overflow lesson, second occurrence); it is full-width now. Two bold headings, *Interactions* and *Models*, split the one box the way 22g-iii dissolved the CI-methods sub-box. In a card the interactions follow the plain predictors after a `│`, labelled `a × b`; ticking one **greys and unticks its two parents** (`title="included in age × race"`) and unticking restores them, in place — no re-render, or the tick would destroy the card it was made in. A new card holds the full pool and **no** interaction: the additive model is what a with/without comparison starts from.

**What re-renders, and what must not.** `modelsSig` is unchanged (`[pool, outcome.length]`), so `models` and `crosses` stay outside it. Add / delete / a `<select>` pick change what the OTHER half shows — a card's chip labels, a row's own option list — so those three re-render synchronously in their handlers, which is safe under 22g-iv's rule (a `<select>` has already committed and lost focus when `change` fires) and is what fixes the stale-drop-down bug. A tick and a name keystroke repaint in place. `onChange_vars` loses its long-standing inconsistency for free: it never called `renderCrossPicker`, and there is no longer one to call.

**`EFFECT_OF_RADIO` went with the maintainer's own in-flight fix.** `effect` became a ComboBox, so the map named four controls that no longer exist — which `test-jamovi-vocabulary.R`'s `*_OF_RADIO` gate and `ui_bracket_names()` both catch. `applyModelEnables()` greys `measure` only; a ComboBox cannot grey one of its own items, the same price `display` paid in 22g-iii, and an unavailable combination is refused R-side, by name, with its cures listed.

**Tests.** `test-reg-cross.R`'s jamovi block is rewritten around the new rule and split in two: the picker's keys (`a*a` refused, an exact repeat deduped, `a*b` kept beside `b*a`) and the fold (no card → the pair replaces its parents; a card holds it only when ticked; a parent named beside its own key dropped; an undefined key dropped). `test-jmvtabreg-cache.R` gains a card with no `crosses` field at all (the shape every card had before), a card holding only an interaction, and both arms of the `jmvtab_reg_staged()` trap. Full suite **FAIL 0 | PASS 9666**, and **no golden or snapshot moved** — nothing in the phase reaches a rendered table.

**The build chain ran here** — `md5sum` → `dev/generate_jamovi_js.R check` (no drift; nothing shared was touched) → `node --check` → `jmvtools::prepare()` → `md5sum` again (all four yaml **unchanged**, no `yaml.dump()` rewrite) → `i18nUpdate("catalog")` → `i18nUpdate("fr")` → `install(home = "flatpak")`. **French: 207 of 208 translated**, the one left being the package DESCRIPTION blurb; only one msgid is new (the box's own label).

**A second round, after the maintainer's live pass, closed five more items.**

⚠ **`race*age` printed "effect of race within each level of age" about a table that says the opposite.** `reg_cross_resolve()` silently reads it as `age*race` — the rows are about the FIRST variable, and only a continuous one has slopes to show within groups — so the picker was describing a pair R had already flipped. `crossOrder()` now puts every pair in that order where it is stored, which is why it needs no message: the two column heads already explain which is which, and R then has nothing to announce. ⚠ it must mirror R's own `kind()`, which reads the column **after** `shape`: a number cut into groups IS a factor there, so `crossKind()` checks `cachedLevels()` **and** the stored shape against `TABX_SHAPES_CUT`; while a column is still being fetched the kind is unknown and nothing moves, because a wrong guess would flip a pair typed on purpose. ⚠ and a swap renames the key a card stores, so `syncCrosses()` returns `{keys, renames}` and `reconcileModels()` applies the renames FIRST — otherwise the card finds its key undefined and silently drops the interaction. Nothing changed R-side: the two-continuous autocut, the squared-predictor abort and the continuous-moderator abort were each re-run and each already names its cure.

⚠ **A variable could be put in two interactions, and the second one just did not apply.** `a*b` beside `b*c` is a three-way interaction, which tabxplor does not fit. A variable another row uses is now simply not offered (`crossClaimed`), `+ Add interaction` seeds the first two FREE variables and greys out below two, and `syncCrosses()` drops a colliding row loaded from an older analysis. That is the third member of the same rule — the picker cannot express what the producer refuses — beside `a*a` (which swaps) and the order above.

⚠ **The delete `×` was unreachable because a `<button>` in a grid track overflows the row.** A grid item defaults to `min-width:auto`, so the button's intrinsic width widened its own `24px` track and pushed the row off the right edge of a narrow pane. `TABX.xDel` sets `min-width:0; width:100%; padding:0`, and the two `<select>`s narrowed to `minmax(48px,1fr)` at `gap:6px` — the options pane is ~340px at its narrowest and the row must fit there.

**The greyscale is one declared ladder, in literal hex, and every list is a WELL.** The `TABX` boxes used `rgba(0,0,0,0.0x)` overlays, so one key rendered two shades depending on what it sat on. They now read the ladder the shared per-variable table already used, recessed → raised as material reads elevation in a light theme: `#CCCCCC` a header row in the SHARED per-variable table, whose heads label a grid of cells · `#DCDCDC` a **well** · `#ECECEC` a **card** raised on it · `#FFFFFF` an **input**, reserved for selects, text boxes and buttons, which is what makes them read as inputs inside a grey pane. ⚠ a head standing over CARDS takes no fill at all — both `outcome` / `family =` / `link =` and the interactions' *effect of* / *within each level of* sit directly on the well, because a filled bar there reads as one more row of the list rather than as its heading; the second pair is additionally italic at the row's own type size, being a plain caption rather than an argument name. Both lists — the interactions and the model cards — and the per-outcome Model table are now the same object: a well holding its rows **and** the `+ Add` button that grows it.

**Two verbosities cut, on the maintainer's call.** A card's tick-box is labelled with the key itself — **`age*race`**, `tab_reg()`'s own spelling and the name the var column, the footer rows and `reg_formulas()` all print — so a jamovi user reads the R syntax rather than a prettified twin of it; the row operator is `*` for the same reason. And the two prose notes went: the "One model: race*age + rincome + …" line that restated the fold, and the "A model holding the pair does not hold its two variables separately" line under the rows. Only the EMPTY interactions state keeps a sentence, because there is nothing else to read there.

⚠ **A third round root-caused a bug that has nothing to do with interactions: `Run comparison` did nothing, and both live caches were silently degraded.** `jmvcore::Image$new()` defaults **`clearWith` to `"*"`**, and `ResultsElement$fromProtoBuf()` opens with `if (someChanges && identical("*", private$.clearWith)) return()` — so a `$state` carrier is **not restored the moment ANY option changes**, which is the only thing that makes an analysis re-run at all. The staged flow is exactly that: the click writes `run_compare = TRUE`, the `.js` writes it back to `FALSE` two seconds later, and on that second run `compare_state$state` is `NULL`, so `.run()` takes its `is.null(last)` branch and paints the "Model comparison staged" banner over the table the trigger run had just computed. **`clearWith: []` on all three carriers** (`jmvtab`'s `cache_state`, `jmvtabreg`'s `cache_state` and `compare_state`) fails both guards and the state comes back; these stores are the module's own and are invalidated by SIGNATURE, never by jamovi's option diff, so clearing them on an option change is precisely backwards. ⚠ and the same line is why the two live caches could only ever hit when nothing had changed. ⚠ **`visible: false` was not the cure and 22g-vi's reasoning for it was wrong**: `Image$asProtoBuf()` sets `ANALYSIS_RENDERING` on a state-holding image that wrote no file *whatever* `visible` says — the branch never reads it — so hiding the carrier could not have stopped a render round-trip. It is kept (no vertical space, and `state` rides a branch that ignores it), but the flicker it was credited with fixing was this.

**Still open: the maintainer's live click-through of round 2** — the two wells at narrow pane width and the delete `×` now reachable; the new greyscale read as elevation rather than as stripes; `race*age` landing as `age*race` in the row itself; a variable in one interaction unavailable to the next, and `+ Add interaction` greying out; a chip tick greying its two parents and unticking restoring them; a with/without comparison and its LR footer row; and `effect`'s ComboBox still greying the `measure` radios.


#### Phase 22g-ix — the results panel sizes itself from the table — DONE

**One CSS declaration was missing, and it is why every jmvtab / jmvtabreg result has been exactly 620 px wide whatever the table.** jamovi sizes an analysis FROM its results iframe — the iframe posts `#results.getBoundingClientRect().width + 40` and the app obeys it with a **620 px floor and no ceiling** (`main-*.js`, `case "sizeChanged"`) — and `#results` is `display:inline-block`, so it hugs its content. But jamovi's own stylesheet **pins an Html result at `.jmv-results-html{width:500px}`**, one rule AFTER the `.jmv-results-item{width:max-content}` it gives every other item. A definite width contributes exactly itself and an overflowing descendant contributes nothing, so the table's real width never reached the host: 500 + 24 + 24 + 40 = 588, clamped up to 620, and everything past it clipped by an iframe that is `scrolling="no"` — the scroll box's own scrollbar with it. That is the reported symptom exactly: *cut before the end, and the scrollbar cut too.* Read from the installed 2.7.36 bundle, not inferred; the full chain with every rule quoted is **`dev/jamovi_results_width.md`**, and guide §7.3 is rewritten to point at it.

**A visible `Image` "fixed" the width because an explicit px width is definite and in-flow** — the 2.6.44 capture shows the tabxplor analysis iframe at `width: 1168px` beside two neighbours at the 644 px floor, driven by the old `plot` Image alone. It is a side effect, not a mechanism, and it costs the vertical space the maintainer measured. The carriers stay `visible:false` for that reason and no other.

**The cure is to un-pin, not to cap** — `.jmv-results-html{width:max-content;}`, which restores the framework's own intent one rule earlier. `width:max-content` also makes the box's min-content equal its max-content, so `#results`' shrink-to-fit lands on the true width in ONE pass and cannot oscillate with the resize feedback loop. **The display cap is gone** (maintainer's call): the results panel is `overflow:scroll`, so a wide table scrolls the PANEL exactly as jamovi's own wide tables do — one scrollbar instead of two nested ones (on a 1920 px screen the old 1600 px cap was WIDER than the panel, so it would have produced both), and jamovi's own results export keeps the whole table. `max-width` survives only as a 4000 px runaway guard, and `@media print` lifts even that. Nothing is ever padded: `max-content` imposes no width, so a narrow table is exactly as wide as it is.

⚠ **Un-pinning means prose starts driving the width, and that had to go with it.** A wrapping block's max-content is its WHOLE text on one line, so the Regressions placeholder (~200 characters) would have reported ~1300 px with no table on screen. Hence ONE boundary: `jmv_results_content()` (`R/jmvtab-export.R`) emits the style once and then the fragments, every non-table fragment goes through `jmv_results_note()` (class `tx-note`, `max-width:520px`), and all five `html_table$setContent()` calls in the two backends go through it — the export status line and both regression placeholders included, so no backend hand-writes a `<div>` any more. `tab_render_scrollbox()` moves out of `R/tab-render-html.R` (it was jamovi-only) and its device-width cap tiers go with the cap.

**The same defect, one layer in: `.tabxplor-caption`.** Its comment claimed it wraps at the table's width and "never forces extra width, its max-content being the longest word" — which is not what max-content means for a wrapping block. Inside a shrink-to-fit box a long title SIZES it. Fixed with the package's own idiom, the one `.tx-foot` has used since 14j: `width:0;min-width:100%`.

**Tests.** Three gates in `test-jmvtab-export.R`, each aimed at the way this defect is silent: the chrome is emitted once and in front and the box is capped only by the guard · **no `.b.R` line hand-writes a `<div>`** (verified non-vacuous: it is the only reach into two private R6 methods) · **every `setContent()` goes through the boundary**, since a new code path writing the element directly would re-pin the panel with no R-side symptom. Plus the caption rule in `tab_css()`. Full suite **FAIL 0 | PASS 9683**, and the only snapshot line that moves in the whole phase is that caption rule.

**No `.a.yaml` / `.u.yaml` / `.js` / `.h.R` change, so no `jmvtools::prepare()`** — `jmvtools::install(home = "flatpak")` shipped it.

**Still open: the maintainer's live pass** — a narrow table (the iframe should sit at the 620 px floor), a ~1200 px table (iframe ≈ its width, nothing clipped, no scrollbar at all), a very wide regression table (the panel scrolls, one scrollbar), the staged-comparison banner and the empty-selection hint on their own (neither may blow the panel wide), and an Excel export with a long path.


#### Phase 22g-x — the crude block's own cache, and a reorder that is display — DONE

**Two of the three costs Phase 22j left behind are gone, and the third turned out to be one line of arithmetic in the wrong function.** The measurements are `dev/benchmarks/results_2.0.0/phase22gx_crude.txt`, taken by `dev/benchmarks/phase22gx_crude.R` — which takes a package path, so the same script measured a pristine `git archive HEAD` tree and the BEFORE column is not a memory.

**The observed (crude) fits ride the store, on the same tier, under the same seam.** `reg_empirical_fit()`'s per-predictor loop already called the SAME `reg_fit()` the model path calls, so the work was a key shape and a digest-readiness fix, not machinery. The fetch/distil/rehydrate dance that was written inline in `reg_spec_build_one()` is now **`reg_fit_cached()`** (`R/reg-digest.R`), and both callers go through it. A crude key is a **synthetic one-predictor spec** whose `extra` leads with `"crude"` (so it can never collide with a one-predictor MODEL spec) and carries `method` / `multiplier` / the shape or cross term — and no estimand, the whole point being that `measure` / `effect` / `conf_level` are answered from the record.

**Measured** (gss_cat, 21 483 rows; the crude share of a SERVED build, `empirical` TRUE minus FALSE):

| case                             | crude before | crude after | served build   |
|----------------------------------|--------------|-------------|----------------|
| binomial, factors + 1 numeric    | 0.071 s (27 %)| 0.038 s (16 %)| 0.266 → 0.235 s |
| binomial, 3 numeric predictors   | 0.070 s (55 %)| 0.030 s (34 %)| 0.127 → 0.087 s |
| binomial, marginal RD            | 0.251 s (55 %)| 0.219 s (51 %)| 0.454 → 0.428 s |
| multinomial, factors + 1 numeric | 1.858 s (51 %)| **0.127 s (6 %)** | **3.624 → 2.009 s** |
| multinomial, marginal RD         | 30.97 s (95 %)| 16.73 s (91 %)| **32.72 → 18.48 s** |

So the phase is worth it exactly where it was scoped to be: **a multinomial's crude block is ×15 faster** and its served build ×1.8, while an ordinary binomial saves ~0.03 s — nothing a user can feel, and said so rather than dressed up. ⚠ **The marginal rows are the honest limit**: only the FIT is cached, and a marginal crude row is a `reg_marginal()` sweep per predictor, which IS estimand-dependent. A multinomial marginal table is still the panel's slowest shape by an order of magnitude (18.5 s), and caching the SWEEP would need a second key carrying the estimand.

⚠ **A crude record served from the store carries a digest and NO fitted object, and three sites demanded one** — the Phase 22j lesson repeated, and all three silent. `reg_gap_se_columns()`'s numeric arm gated on `nv$fit` and passed `nv$fit` to both AME influence makers; `reg_empirical_fit()`'s marginal arm passed `f$fit` to `reg_marginal()` with **no `refit` callback**, so the honest abort that function raises for a digest would have been swallowed by the loop's own `tryCatch` and become a MISSING crude row. All three read `reg_model_of()` now, with the callback beside them. **Verified by re-breaking the gate**: with `nv$fit` back, the marginal binomial's `gap_se` goes 10 non-NA to 9 — exactly the numeric predictor's row, i.e. `color = "adjustment"` on every numeric predictor, lost on every build. ⚠ note the cold path loses it too, because `jmv_store_cached()` returns the value it stored: with a cache present a record is distilled on the MISS as well.

**`drop_extra` is a key member now, and its columns are fingerprinted — which closed a latent blind spot on the model path.** `jmvreg_fit_key()` hashed only the outcome, this spec's own predictors and the design variables, while `na_shared_vars` rode in `extra` as NAMES; a value edit to another model's predictor column changes THIS model's complete-case set and did not move the key. It is a named, fingerprinted member for both paths now — and it had to be, since a crude fit's `drop_extra` is *the whole predictor set minus this predictor*, so the same `(outcome, v)` under a different model is a different fit. `JMVREG_CACHE_SCHEMA` → **9**.

**The gate is two functions because its two clauses are about two different things.** `profile` refuses ANY fit (its bounds are a likelihood output, not rebuildable from estimate and SE); a model comparison refuses only the MODEL fits, being a test between the fit objects. A univariable crude fit takes part in no comparison — so `reg_crude_cacheable(method)` is the profile clause's one home and `reg_fit_cacheable()` composes it. The payoff is real: **a staged model comparison, which bypasses the model store by design, now serves its crude block.**

**`reg_fit_cached()` also carries one new guard both paths get.** `reg_digest_frame()` returns NULL when the rebuilt frame is not the fitted one — a refusal, because the domain moved. Downstream that read as "no gap SE", silently; the seam now recomputes instead of serving a record with no frame.

---

**A level reorder is DISPLAY, and the roadmap's fear of it was based on a premise the code already refutes.** `reg_fit_frame()` strips `ordered` from every factor PREDICTOR before the fit, with its own comment saying why (polynomial contrasts no per-level skeleton can align), and there is no `contr.poly` call anywhere in `R/`. So **every predictor is fitted under treatment contrasts, in every family**, and its level order decides exactly one thing: which level the others are compared to. That one thing is `ref =`, which relevels the data and is an honest refit. The rest is a permutation of rows.

`tab_reg()` therefore gains the internal **`.levels_order`** dot-arg beside `.levels_collapse`, and it is applied by **`reg_skeleton_reorder()`** to the row SKELETON — at both places a skeleton is born, so the compound-formula path (`reg_skeleton_from_fit()`) is not left fit-ordered. ⚠ the reference row stays FIRST and only the rest are permuted: `is_ref` and `term` were built positionally by `reg_skeleton()` (`term = paste0(p, lv[-1])`), and every later stage reads them as flags. Unlisted levels trail in their own order — `fct_relevel()`'s contract, hence the panel's. It is safe because every fit→row alignment is by KEY (`reg_skel_match()`, `match(skeleton$term, td$term)`) and every row semantic is a flag; the row axis is an unsorted copy of the skeleton (`forcats::fct_inorder`), so one permutation moves the printed rows and the `tabxplor_lvl` level set together.

`jmvtab_reg_build()` accordingly **stops calling `jmv_relevel_cols()`** and passes the order through instead, translated to merged names by the same `jmv_order_after_collapse()` jmvtab uses (the skeleton is built after `.levels_collapse` has run). **No `.js` change at all**: `varSync` still writes `ref := order[0]` and `regRefToFirst()` still moves a picked reference to the front, so `order[0] == ref` stays true, `boldFirst` stays right and no widget moves.

**Measured, multinomial, on a warm store**: a reorder below the first level goes **14.8 s → 1.85 s (×8)** and is a cache HIT; moving a level ONTO the first is unchanged at 14.7 s and 0 hits, because it is the baseline and the model really did change. Binomial: 0.315 → 0.220 s. Blast radius for R users: **zero** — `.levels_order` defaults to NULL.

⚠ **One stated behaviour change**: the data relevel used to reach `reg_cross_column()`, which bakes both parents' orders into a materialised combined factor, so a ▲/▼ move also permuted an interaction block's rows. It no longer does — a combination's row order is `expand.grid`'s. Honest (that block's rows are pairs, not either parent's levels), and narrow.

---

**The ▲/▼ greying on an ordered factor was already built, and already shared.** `tabxmBuildList()` takes `canOrder = mtypeCache[v] !== "ordinal"`, disables both buttons and the arrow keys, paints them `TABXM.btnOff` and titles them *"an ordered variable already has the order its levels mean"* — inside the `BEGIN/END SHARED` span, so `jmvtabreg` carries it byte-identically. Nothing was written. The only thing that can leave the arrows live on a variable the maintainer considers ordered is that column's jamovi **`measureType`** not being `Ordinal`, which the JS cannot see past; it is on the live-pass list, with the variable name asked for. (After this phase the greying is a statement of MEANING only — a reorder could not move a number even if it were allowed.)

**The drop-down latency is the analysis re-run, not the widget, and the measurement says so.** `fetchLevels()` caches per variable and never repeats; a `ref` pick's own handler is a bare option write; and every var-table control writes an option that is in `.opts()`, so a pick re-runs the analysis. The interaction table above IS that latency. Two picks stay slow for a reason (`ref` and `shape` change the model), and **one does not**: `multiplier` is a REPORTING choice, yet it sits in the fit key because `reg_tidy_rescale()` runs inside `reg_fit()` and `tidy_native` comes out already scaled. Moving that rescale into `reg_tidy_finalize()` would let it leave the key exactly as the estimand did in 22j (**14.7 s → ~1.9 s** on a multinomial). It interacts with the profile bounds, which ride natively on the record, and with `reg_digest_revive()`'s recipe — **recorded under Phase 22x, not built here.** No `.js` was touched, so no `jmvtools::prepare()`.

**Tests.** New in `test-jmvtabreg-cache.R`: a crude fit is served and the served table is the cold one · an all-factor block on a flat design stores NO crude record (saturated, so nothing was ever fitted) · a served record keeps its gap SE, weighted and unweighted, with the NUMERIC predictor's own row asserted · a marginal crude sweep is the same served as cold · `drop_extra` is in the key by name AND by value · `profile` serves nothing · the model-comparison block gains its crude arm. In `test-reg-digest.R`: the two-predicate gate, and `reg_fit_cached()` with no store / no key / both. In `test-tab_reg.R`: `.levels_order` permutes the rows and changes no estimate, on a binomial AND an ordinal outcome with an ORDERED predictor, and an unknown level or variable does nothing. Full suite **FAIL 0 | PASS 9757**, and **no golden or snapshot moved** — every path in the phase is behind a `fit_cache` (never present outside jamovi) or `.levels_order` (default NULL).

**Still open: the maintainer's live pass.** A multinomial with a numeric predictor — a `measure` change and a ▲/▼ move should now feel like the same fast re-paint, while a `ref =` pick stays the slow honest one; `color = "adjustment"` keeping its gap band across a re-paint; an ordered predictor's greyed arrows (please name the variable if they are live); and an interaction block's row order, which no longer follows a parent's ▲/▼.


#### Phase 22g-xi — the shape table names the transform it drew — DONE

**`shape = c(age = "sqrt")` always reached the model; the one place a reader judges a shape did not say so.** The maintainer read it as "`shape =` does nothing". Measured, it does everything except announce itself: the column is recoded in place, the row label reads `√(x), per 2.09 (2SD), at 6.42`, `reg_formulas()` shows the fitted formula and the odds ratio moves (2.14 → 2.29 under `sqrt`, → 2.46 under `log`); `quadratic` adds `I((age/13.46)^2)` and a second row at `1/1.34***`. What was missing is one word: the shape table's predictor column said `age` whatever ruler the curve had been drawn on.

**The curve was already right, because a transform recodes the column in place.** `shape_apply()` writes the transformed values back under the same name (`R/var-shape.R`, *"The column keeps its NAME"*), so `reg_curves()` was already binning `log(age)` — the drawing genuinely re-rules its own axis, and only the label lagged. Hence a **name-marks-only** change: `log(age)`, `√(age)`, from the one `SHAPE_MARKS` row that already spells them.

⚠ **`quadratic` gets no mark, and that is the load-bearing decision.** It adds a model TERM and recodes nothing, so the observed curve is unchanged — and marking it would promise a "look again, is it straighter?" reading the drawing cannot support. Measured: `y` binned against `age²` is still a hill (`▁▆███████████▇▇▇▆▆▆`), so that check would REJECT a parametrisation which in fact fits (`age²` is `1/1.61***`). Worse, `x²` is not even monotone on a variable spanning zero: on centred age two values fold into one bin and the y-values themselves change (`█████▇▆▅▄▃▂▁▁▁▁▁▁▁▁▁`), a different curve about a different question. A quadratic is judged by `reg_check_plots(check = "linearity")`, which draws the fitted parabola THROUGH the observed points — the one comparison that is honest for it.

**The rule fell out of the fact table rather than being written.** `shape_mark()` returns `NA_character_` for every kind whose `VAR_SHAPES` row carries no `mark` — `linear`, `quadratic` and all five cuts — so "no mark for a term, no mark for a cut" needed no clause; verified for all eight kinds. `reg_pred_facet()` (`R/plots.R`) already answers the identical question the same way and was the precedent followed.

⚠⚠ **The mark had to go on the display cell and nowhere upstream.** `names(rec$curves)` are load-bearing KEYS in three places — `reg_bind_assumptions()` unions them across `tab_vars` groups, `linear_level` matches them against `skeleton$var`, and `mat_reg_spark()` matches them against the tab's own `var` column — so marking the names would have broken all three silently. The mark is stored beside the curves at build (`a$mark`, in the same `purrr::map` that already builds `a$linear_level`, where `shape_kinds` is already a bare local) and read only where the `var` CELL is written. It survives the grouped path for free, `reg_bind_assumptions()` keeping `rec <- ad[[1]]`. ⚠ read with single-bracket `rec$mark[v]`: `[["nope"]]` ERRORS on a named vector while `["nope"]` is `NA`, which is the fallback a group lacking a variable — or a table built before this change — needs.

**No renderer changed.** All four consumers treat `var` as plain text and derive their widths from it: console and markdown share `tx_pipe_table()`, html escapes it and keys the SVG upgrade on the column NAME, Excel writes a bare cell and counts rows rather than characters. Verified end to end on one table in all four media, plus a `tab_vars` split (the mark on every group) and a two-outcome table with two different shapes. ⚠ Accepted caveat: `√` (U+221A) is East-Asian **Ambiguous** width, so `nchar(type = "chars")` counts 1 while a CJK-wide terminal font draws 2 and shifts the pipe table's later columns — the same exposure the row label's `√(x)` already ships; the fallback, if it ever bites, is one `SHAPE_MARKS` value. ⚠ Left alone, pre-existing: `xl_col_widths()` measures the MAIN table only, so the shape table's columns inherit its widths and a marked name may overflow into an empty neighbour.

**Why `log(age)` here while the row label keeps `log(x)`.** Not two spellings of one fact but one rule applied on both sides (`R/var-shape.R`): the model row sits BESIDE a `var` column that names the variable, so it writes the literal `x`; in the shape table that column IS the name, so it writes the variable.

**Tests.** ⚠ The load-bearing repair is `test-reg-shape.R`'s sparkline test, which looks a curve up by `st$shape[st$var == v]` and then builds a table with `shape = c(age = "log")`: after the change that lookup returns `character(0)` and its `expect_false(identical(...))` keeps **passing vacuously** — the file would have stopped asserting "a monotone shape moves the curve" with nothing red. Keyed on `"log(age)"` now, and the helper gained an `expect_length(out, 1L)` so it cannot go vacuous again. New locks: the three marks and the bare name, that a quadratic's curve is byte-identical to the untransformed one, that a cut yields no shape-table row at all, and that the mark survives the `tab_vars` merge WHILE `names(curves)` stay raw. ⚠ The quadratic lock was verified by re-breaking it (a `mark` added to the `VAR_SHAPES` quadratic row): it fails, naming the line. Full suite **FAIL 0 | PASS 9746**, and **no golden or snapshot moved** — no `_golden/` or `_snaps/` file holds a shape table, and `dev/make_golden.R` builds only crosstabs.

**The article.** `dev/tabxplor-the-shape-of-a-number.Rmd` (new, `dev/` only for now) teaches reading the observed shape and curing it, for the literary student. Its central instruction — *set a shape, look again, is it straighter?* — is now scoped to `log`/`sqrt`, with `reg_check_plots(check = "linearity")` taught as the check for `quadratic`, its figure evaluated rather than described. ⚠ Reading that figure honestly changed the prose: on `age → married` the fitted parabola gets the hill right and visibly misses its asymmetry (the real rise before 35 is steeper than any symmetric curve that still falls at the far end), so the section now says what it shows — *a cure is an improvement, not a fit* — and sends a reader who needs the exact peak to a quantile cut instead.


#### Phase 22g-xii — the shape table says what its two axes are — DONE

**The drawing had no axes, so a reader could not say what its height meant.** 22g-xi named the horizontal one (the predictor, marked by its `shape`); this names the vertical one. The first column now spells the outcome on the scale the MODEL fits, exactly as `reg_check_plots()` labels its y axis — so the small table and the panel state one quantity, and a reader who has met `log(%Married / (1 - %Married))` in one meets it in the other.

**One form per link, derived from a fact table that already declared the distinction.** `rd_link_text()` is the plain-text twin of `rd_link_expr()`'s plotmath: same five kinds, and the SUBJECT comes from the one declared field both read, `RD_LINK_SCALES$level` (`pct` = a percentage of the modelled level, `num` = a mean of the outcome). Measured across every family:

| the model                       | the column reads                                 |
|---------------------------------|--------------------------------------------------|
| binomial, logit                 | `log(%Married / (1 - %Married))`                 |
| binomial, `link = "ratio"`      | `log(%Married)`                                  |
| binomial, `link = "difference"` | `%Married`                                       |
| gaussian                        | `mean prestige`                                  |
| poisson                         | `log(mean tvhours)`                              |
| ordinal / multinomial           | `log(%rincome not 1st / (1 - %rincome not 1st))` |

⚠ **The ordinal / multinomial cell had to name its outcome, and that killed a duplicate.** `rd_link_y()` reads those two as `Y != first`, so there IS no modelled level to name — and the subject must still carry the outcome, or a several-outcome table would show two rows nothing distinguishes. `%x not 1st` says both, and is right for both families at once (on an ordered outcome `Y > 1st` and `Y != 1st` are the same set). It then makes the ` (1st curve)` suffix 22g-v put in that cell say the same thing twice, so the suffix is **deleted** — its test keeps its name and asserts the new wording.

**The header is `outcome (model scale)`**, and the modelled level is STORED on the record (`level`, binary families only) rather than the rendered string, so `gettext()` runs under `with_legend_lang()`'s language and not the one that happened to be current at build. ⚠ Keys stay the raw outcome NAME throughout — the sort, the run-blanking and the per-cut test are all about which outcome a row belongs to, not about how its cell reads.

**Translations regenerated** through the sanctioned `dev/update_translations.R`. Five msgids are new or changed and all five are translated by hand: `variable expliquée (échelle du modèle)`, `moyenne %s`, `%%%s hors 1re`, and the two formulas unchanged (a formula is a formula in both languages). ⚠ Two arrived FUZZY carrying an inherited translation that was actively wrong — `log(%s)` had picked up `log(risque)`, which would have printed the word "risque" where the outcome's own level belongs. Verified in both languages end to end. The three `tools::checkPoFile()` warnings left are pre-existing `%`-in-prose false positives in `plots.R` / `reg-estimand.R`, untouched here.

**Tests.** One new block pins the six forms and asserts the table's cell IS `rd_link_text()`'s output, so the panel and the table cannot drift. Two existing blocks were updated rather than deleted, both keeping the fact they were written for: the ordinal one still asserts "an ordinal outcome says which curve it is drawing" (now via `not 1st`, plus that it names the outcome), and the two-outcome one still asserts one row per outcome, each naming its own — by match rather than by equality, since the two rows are no longer even in the same units. Full suite **FAIL 0 | PASS 9755**, no golden or snapshot moved.
...
#### Phase 22h — miscellaneous manual reviews — DONE

**Nine review items; three had a false premise or an existing cure, and saying so is part of the phase.** Every one was reproduced against real data before a line was written. The through-line: each item was a fact already declared somewhere and simply not read at the point that decides — a scale's level twin, a column's role, a template's bracket group — so the phase moves reads rather than adding rules. The one new capability is `a*b` in `col_vars`, which is the bulk of it.

**A predictor comparison states the level ONCE, by the observed column** (item 1). `reg_display_of()` returned `est_base` whenever a crude companion existed, so both model columns of a `list(a =, b =)` table carried `(48%)` between them — a bracket between the very numbers the spelling exists to compare. One new `DISPLAY_PRESETS` row, **`est_base_once`** (`c(default = "{est}", emp = "({base}) {est}")`), so the role split stays in the one table `display_preset_arm()` reads and a post-hoc `set_display()` reproduces it; `reg_display_of()` gains a third argument, and its five call sites already had `is_comparison` as a plain local (`reg_ctx_locals()` puts it there). ⚠ `reg_meta_aside()` had to follow, or the footer's `Model:` line would keep glossing a bracket the cells no longer print — and it tests the BUILD's comparison (`meta$comparison` **and** more than one spec), since a one-model list takes the ordinary layout. `empirical = FALSE` already gave a bare `est`, so only the default path moved.

**A quantile group's rank is readable off its own interval** (item 2), so the `Q<n>` tag is gone — one line, `shape_cut_quantiles()`. `sd_bands` keeps its band words, and the rule is now stated in the header: a label says nothing the bounds already say. Nothing keyed on it (the jamovi cache keys the resolved spec, the regression replay re-measures).

**The shape-table option already existed** (item 3). `tabxplor.spark` has governed `tab_wants_shape_table()` — the one gate every medium goes through — since 22g-ii; only the NAME was stale, the sparkline having stopped printing in the `n` cell. Renamed **`tabxplor.shape_table`**, `alias = "spark"` (read first, so existing code keeps working), `tx_spark_mode()` → `tx_shape_table_mode()`, default unchanged. Then the survey the item really asked for: `?tab_reg`'s examples and every vignette but the section that TEACHES the shape table now set it to `"no"` — a setup-chunk line in a vignette, a save/restore pair in the roxygen block (R CMD check runs a file's examples in one session), and `"all"` re-enabled around *The shape table* / *Curing a shape* in `tabxplor-reg.Rmd` and its French twin.

**Excel: the unit tag is the one header piece that does not wrap** (item 4). It shared one measurement with the level header — same size ratio, same two-line budget, brackets counted — so `<obs mean>` could both widen its column and break mid-name. It is now measured on ONE line at its own size (`XL_UNIT_SIZE <- 8`, against a body of 10) with the angle brackets excluded from the count, written at that size, and `wrap_text` is off on that row. ⚠ the index columns' header is merged down over it and takes the top-left cell's style, so the level header keeps its own wrap.

**And the tag a `Model_*` column had lost** (item 5). Excel alone runs `mat_aside_cols()`, which splits a composite cell into its primary and its asides; an `Obs_*`/`Model_*` pair whose layouts are deliberate MIRRORS becomes four columns of one block — `aside:emp | emp | model | aside:model` (measured) — and the two estimate columns then carry the identical tag string, so `tab_units_once()`'s `(block, unit)` run swallowed the second. Always the model one, the crude companion being written first. The column's **role** joins the run key: two columns of different roles are different columns whatever they hold. html / md / console never split, which is why the defect was Excel-only.

**`OR: 1` is how a reader finds the column an odds ratio is read against** (item 6). The `or` tooltip line dropped a whole column of 1s as "saying nothing" — precisely the `ref2` baseline column, the one thing the cells cannot show. The clause is gone; no emphasis question arises, a tooltip being plain text throughout, and the `not_ref` collapse belongs to the pct axis's own reference, not to this one.

**`a*b` in `tab(col_vars =)`** (item 7, the bulk). The principle is `reg-cross.R`'s own — *an interaction is a variable whose levels are combinations; prepare it as one and there is nothing to integrate* — so `tab()` reuses the whole surface (the peel before tidyselect, `reg_parse_crosses()`, `reg_cross_autocut()`, `reg_cross_resolve()`, `reg_cross_column()`, `REG_CROSS_ARMS`) and the new `R/tab-cross.R` says only what each arm means as COLUMNS:

- **`cells`** (factor × factor) — the combined factor is an ordinary categorical col_var, one column per observed cell, both parents absorbed. The chi², the intervals and the colour needed no code of their own.
- **`nested`** (numeric × factor) — the moderator keeps its own block (`keeps = "moderator"`), then one MEAN COLUMN PER LEVEL, materialised as one numeric column each (the modified variable where the moderator is that level, `NA` elsewhere). Measured byte-identical to what `spread_vars` builds on the same data, and each column's base count is its group's own — `num_moment_scan()` already excludes NAs from both the mean and the `n`.
- **two numerics** — `reg_cross_autocut()` unchanged, with its one-line message.

**The pipeline's own division of labour carries it** (`R/tab.R:6-17`: *`tab_setup()` builds the settings spine and `tab_prepare_pop()` completes it*). The arms are decided at the boundary on the SHAPE PREDICTION — the same question the col_var classification asks, so `reg_cross_resolve()`'s `kind()` now reads `shape_is_factor()` beside the column's class, which is true in `tab_reg()` too where it runs after the recode. One placeholder column per pair enters `col_vars`; `tab_prepare_pop()` materialises what each arm makes and **expands the spine's `cols` / `pairs` slices** — the first point a cut moderator has levels, which is why it cannot be done at the boundary.

⚠ **Four traps, each found by tripping it.** (1) `na = "drop_all"` would list every synthetic nested column and empty the table, no row being non-NA in all of them: the set takes the cross's PARENTS (`reg_cross_parents()`, the same function that feeds `tab_reg()`'s complete-case frame). (2) The refresh after the expansion must be COLUMN-grain only — a blanket `list2env(ctx_settings_locals(ctx))` clobbers the `na_text` / `na_num` just computed above with the empty ones the spine does not hold yet. (3) A `cells` column is REBUILT after `shape_apply()` where a parent was cut: a combination of raw values is not a combination of groups — which also means the parents must reach `sel_vars` and survive `tab_prepare_pop()`'s select, being col_vars no longer. (4) The block is stamped AFTER the numeric/text column sort, which matches `col_var` against `col_vars`; stamping earlier dumps the whole block last.

**The internal column names are syntactic; the typed key is the display identity.** A data column literally named `age*race` aborts `tab()` (measured), so `tab_cross_col()` mints `.tx_x_age_race` and `tab_cross_stamp()` restamps `col_var = "age*race"` once the columns are in place — `tab_col_block_ids()` then reads one block and `tab_col_units()` writes the unit once. A nested column is named `<level>_<key>`: ⚠ the bare level collides with the moderator's OWN block, which prints the same levels as columns of its own, and a duplicate name is not a table — while the export prep already strips a trailing `_<col_var>` off a header, so the column shows `White` under a span reading `age*race`. Refusals, each by name: a key in `row_vars` / `tab_vars` / `spread_vars` (before tidyselect can call it a missing column), in `tab_counts()` (which starts from aggregated counts and has no parents to cross), and a numeric-keeping `shape` on a parent (`shape_rename_transformed()` renames only what `col_vars` names, so an absorbed parent would silently be crossed raw). Every `reg-cross.R` message takes an `arg =`, and the two whose reasoning is model-specific branch on it.

**7b — the prerequisite bug, and it is not about interactions.** `tab(d, marital, all_of("my race"))` **aborted** — *Column `` `my race` `` doesn't exist* — while `tab_reg()` on the same column was fine. `as.character()` on a LIST of symbols deparses each element, so a non-syntactic name came back backticked and every later selection missed it. It is a hard prerequisite here, moderator levels routinely holding spaces (`gss_cat`'s `race` has `Not applicable`). Cure: **`vars_chr()`** in `R/utils.R` and a mechanical sweep of the 104 sites across nine files — plus two `paste0(col_vars, "_mean")` twins in `num_core()`, the same trap through a different function.

**The Total column, and a deviation display** (item 8): two independent defects, and the maintainer's own proposal turned out to be already in the code.

- **8a — the 100 % vanishing under `color = "ratio"`.** Real, but only once a reference interval is on (`color_signif`, `stars`, `ci = "ref"`; with bare defaults `ci` resolves to `"no"` and nothing shows). The colour makes the leaf stamp the INTERVAL's scale, `pct_ratio`, on every column of the block including the Total — `scale_1` is column-invariant by construction — and `tab_totcol_sums()`'s second gate then summed `ratio`: 3.04 against the Total's 1. A difference passed only because it happens to be additive. **Cure**: "do the parts add up to the whole" is a question about LEVELS and nothing else, so it is asked on the column's level twin — a fact `EST_SCALES$level_twin` already declares. `fmt_est_of()` gains `fmt_scale_key()`'s own `kind` argument; every other caller keeps `"auto"`. The two gates now say two distinct things: *is the display a level?* and *do the levels add up?*
- **8b — the count printed in bold and in the table's ink.** Where the block genuinely does not sum, `tab_fold_base_n()` already writes `"({n_range})"` — the maintainer's proposal, shipped. But a template whose every token is bracketed still called its one token the primary, so `format(bold_split = TRUE)` recorded the count's own character range. **Cure**, one clause where the range is recorded: **a template with no token outside brackets has no primary**, and a zero-width range is written instead, which is what `paint_split()`'s existing branch was built for (`html_cell_text()`'s `pn >= 1L` becomes `pn >= 0L`). Its type TAG still reads `n` — the column holds a count and the cell reads it as an aside, two questions. Fields stay STORED: `diff = 0` / `ratio = 1` are exact, users read them with `$`, and no display shows them.
- **8c** — `display = "estimate"` aborted where `"est"` worked: one alias preset row. The regression side needed nothing else, its `n` column keying on the column's `role` and never on the display.

**`shape_name` is `FALSE`** (item 9), opt-in as asked; the read site takes `TAB_ARGS$shape_name$default` instead of repeating the literal.

**Tests.** New: `test-tab-cross.R` (both arms, the block identity through the export prep, the autocut, the eight refusals, the `drop_all` trap, a shaped parent giving combinations of GROUPS, and the non-syntactic-name fix) · the comparison layout and its footer · the two Total-column cures, with the zero-width primary range asserted through `format()` and through the rendered html · the Excel unit row's size, its lack of wrap and its refusal to widen a column · an observed column and its model twin each keeping their tag · the four `shape_table` spellings plus the `spark` alias · `est_base_once` and the `estimate` alias. Updated rather than deleted: the `or` tooltip test keeps its subject and asserts the new rule; the two `var-shape.R` label tests; the preset name list. Full suite **FAIL 0 | PASS 9934**, and the only snapshot that moved is `render-html.md` — verified line by line to be nothing but the added `OR: 1.00` fragments.

#### Phase 22i — last jamovi UI polish and bug corrections — DONE

**Every item had one named cause, and three of them were not the cause the roadmap named.** Each was reproduced before a line was written, and two of the five turned out to be the same defect wearing two faces.

⚠ **The Excel crash is not jamovi's, not the plots', and not about labels being missing.** `purrr::imap()` hands the **NAME** when its input is named and the **INDEX** when it is not — and `xl_check_images()` (`R/tab_xl.R`) built a parallel `labs <- names(grids)` vector and then indexed it with `labs[[i]]`. `reg_check_plots()` returns a bare gtable for ONE model (wrapped here, unnamed → `i` is an integer) and a list **named by model** for two or more (→ `i` is `"age: diff"`), so `[[` on an unnamed atomic vector errored. Reproduced in plain R, message for message: `tab_reg(d, c("age","married"), c("race","tvhours")) |> tab_xl(check = "auto", data = d)`. The label **is** the name, so the vector is deleted and `i` read directly. ⚠ the `%||% ""` guard could never have helped — `[[` *errors* rather than returning NULL on an atomic vector. It shipped because the existing test builds ONE model; the new one builds two and asserts both captions reach the sheet.

⚠ **The same trap, one file over, and reachable: `tab_md(list(a = t1, b = t2), caption = "X")` dropped the caption.** `tab_resolve_tables()` passes a user's list through untouched, so a named list made `i` a string and `R/tab_md.R`'s `if (i == 1)` silently FALSE on every table — `"a" == 1` is `FALSE`, no error, no warning. It iterates the position now. `R/row-model.R:278` had the correct idiom all along and is what both were rewritten against.

⚠ **The roadmap's own note on the ordinal residual panel was stale, and its prescription would have deleted a working panel.** `rd_resid()` has had a correct ordinal randomised-quantile arm since ~94 commits before that note was written (measured: 21 407 finite residuals on a `polr`). The hole is one layer down, in `reg_panel_residuals()` (`R/plots.R`): it takes `as.numeric(stats::fitted(cx$fit))`, and an ordinal fit's `fitted()` is the **n × K category-probability matrix** — 342 512 values against 21 407 residuals — so the length guard fired and `purrr::compact()` swallowed the NULL. `normality` has no such hole and draws today, so dropping `"ordinal"` from its row (as the note asked) would have removed a panel that works.

**The cure keeps the promise rather than retracting it.** `rd_fitted_1d()` (new, beside `rd_resid()` in `R/reg-assumptions.R`, where the family dispatch lives) answers *the one number per row a residual is plotted against*: the scalar `fitted()` for every family, and for an ordinal one the **expected category** — the probability-weighted mean category index, `P %*% seq_len(K)`, length n for `polr` and `svyolr` alike (measured range 6.7–12.8 over 16 categories). The axis is labelled for what it is — one new R msgid, `Expected category`; ⚠ it arrived from `dev/update_translations.R` **fuzzy, carrying an actively wrong inherited translation** (`cette modalité`, i.e. "this category"), the same hazard 22g-xii met, so it was written by hand as « Modalité attendue » and the catalogue re-verified at **0 new fuzzies**. `REG_CHECKS` is untouched, so `check = "auto"` on an ordinal model now genuinely draws the five panels the docs already claimed. A multinomial still returns NULL by the same rule (K columns, no single fitted value), which is the family that genuinely has no residual panel.

**And the gate that makes this class of gap loud.** `REG_CHECKS$<check>$families` is a **promise** `reg_panel_keys()` makes to the user, and nothing checked it — a NULL panel is compacted away in silence. A new test walks every declared `(family × panel)` pair across five families at `check = "all"` and asserts each builds a grob. ⚠ verified by re-breaking the fix: it fails naming the family and the panel (*"ordinal declares 'residuals' and draws nothing"*). A runtime message was considered and rejected — Phase 23c is about removing those, and a test is where a broken promise belongs.

**`ref = "last"` selected the missing values, in both producers, and the rule is now stated once.** `na = "keep"` appends the level literally named `NA` **last** (`fct_na_value_to_level` after `setorderv(na.last = TRUE)`), so a positional reference landed on it — the missing values became the thing every other row was compared to. The rule: *a positional keyword names a substantive **GROUP** of the variable — never a total, never the level a missing value was given; naming `"NA"` still selects it.* ⚠ matching on the name is **exact, not a heuristic**: `fct_na_value_to_level()` MERGES a genuine level of that name into the missing one, so under `na = "keep"` the two cannot be told apart by any means — which is what the new `TAB_NA_LEVEL` constant (`R/row-model.R`) records, read now by all five minting sites.

Its three consumers took the fact the way the leaf already declares `totcol_vector` ("the leaf MINTS this column above, so here the literal IS the declaration"): `diff_index()`'s `is_total` becomes a single **`is_group`** composed at the call site (one concept, no extra argument), while `calculate_refrows()` gains `grouprow_vector` **beside** `totrow_vector` rather than replacing it — ⚠ that one could not be repurposed, because `ref = "tot"` reads it for its own, genuinely different meaning. `reg_ref_level()` applies its keyword to `setdiff(lv, TAB_NA_LEVEL)` *after* the exact-level match, so `ref = "NA"` keeps working. Measured on all three: `60 to 89` where `NA` stood, on the row axis, the column axis and in `tab_reg()`.

⚠ **A second, unrelated defect found on the way, and it made the pairing unusable: EVERY cut shape was refused under `na = "keep_for_predictors"` — including the `sd_bands` the error message itself recommends.** A *resolved* shape spec carries the **canonical** kind (`"quantiles"` / `"bands"`), not the user-facing `VAR_SHAPES` key (`"quartiles"` / `"sd_bands"`), so `reg_na_cut_shapes()`'s hand-rolled `VAR_SHAPES[[sp$kind]]$produces` was NULL for every cut and the abort fired on all of them. The accessor that does that mapping — `shape_is_factor()` — already existed and is used correctly two functions away. One line; the declared fact is read through its accessor again. A numeric-KEEPING shape is still refused, correctly.

**The `ref =` column is headed per group, and the in-cell tag it replaces is deleted.** The per-variable table renders **one grid per group**, each with its own header row, and only column 0 was group-aware. Off the percentage axis (`tabAxisVars`: `col_vars` under `pct = "col"`, `row_vars` otherwise) that column can only ever hold `ref2` — the odds ratio's second reference — so it is now headed `ref2 = <i>(reference)</i>`, and only while an odds ratio is in force; a column that can hold nothing (that axis without one, and **every** table variable) is headed with `""`, because naming an empty column offers a choice the table does not have. The shared span gained one generic line (`g.heads[col.key]`, where `""` IS a head and `undefined` keeps the column's own), so `jmvtabreg`'s single "Predictors" group sets nothing and is unchanged. The inline `ref2 =` span inside the cell then says what the header says, so it goes — the header names the argument once instead of once per row, and the select keeps its full width.

**Tests and translation for it:** the new msgid is the **same wording** with only the argument name changed, so its French is its sibling's (`ref2 = <i>(référence)</i>`); it joins `JMV_WIDTH_BUDGET` at 30 (its 180 px column, 18 visible characters in both languages), and `"ref2 ="` **leaves** `JS_TEXT_EXEMPT` — with the tag gone that exemption is dead, and a dead exemption is a hole in the coverage gate.

---

**`multiplier` leaves the fit-cache key, and the invariant that let it is one sentence: `tidy_native` means NATIVE.** `reg_tidy_rescale()` ran *inside* `reg_fit()`, so the stored tidy came out already scaled by |k| and every scaling pick was a full refit — although nothing in `reg_fit_formula()`, `reg_fit_frame()` or any fitter reads `multiplier`. It now runs in **`reg_tidy_finalize()`**, beside the exponentiation and the interval, because it is a reporting choice exactly as they are; `multiplier` joins `(do_exp, conf_level)` on `reg_fit_record()`, `reg_tidy_finalize()`, `reg_fit_rehydrate()` and the one cache seam `reg_fit_cached()`, and leaves `extra` at **both** key sites (`reg-spec-build.R` and `reg-empirical.R`'s crude one). `JMVREG_CACHE_SCHEMA` **9 → 10**, so a store written when "native" meant something else is discarded.

**Measured** (`dev/benchmarks/phase22i_multiplier.R`, results beside it; BEFORE is a pristine `git archive HEAD` tree measured by the SAME script):

| a `multiplier` pick, warm store  | before        | after         |
|----------------------------------|---------------|---------------|
| binomial, factors + 1 numeric    | 0.127 s, 0 hits | 0.071 s, 1 hit |
| binomial, marginal RD            | 0.133 s, 0 hits | 0.074 s, 1 hit |
| multinomial, factors + 1 numeric | 4.541 s, 0 hits | **0.390 s, 1 hit** |
| ordinal, factors + 1 numeric     | 0.479 s, 0 hits | 0.080 s, 1 hit |

and the rendered table still **changes** in every row: the pick is served, not stale. (The roadmap's 22g-x figure of 14.7 s was a wider multinomial — three predictors, `empirical = TRUE` — so it is not this table's 4.5 s; the ratio is what carries over.)

⚠ **The one silent trap, and it was real.** `reg_cols_ame()`'s odds-ratio **hover** reads `f$tidy_native` directly and exponentiates it; its own comment said "the record already holds on its NATIVE scale", which was true only because native then meant *scaled*. Left alone it would have shown a per-**one-unit** odds ratio beside a row labelled `per 3.08 (2SD)` — a wrong number with no symptom anywhere. It applies the scaling itself now, in the same commit, and the comment says which scale it reads.

⚠ **A latent bug fixed by the move: a NEGATIVE `k` inverted the interval.** The profile branch did `ci * mult_vec` with the **signed** k, so `lo > hi`. `reg_tidy_rescale()` now carries the bounds and re-orders them with `pmin`/`pmax`, and a test asserts `lo <= hi` under `multiplier = c(age = -2)`. ⚠ its own guard reads `names(td)`, never `$`: a tibble WARNS on an unknown column, and a Wald tidy has no bounds yet — that was 482 warnings and one failure before it was spotted.

**Why no number could move:** the p-value is scale-invariant on every arm (`estimate` scales by `k`, `std.error` by `|k|`, so `|est/se|` is unchanged), which the benchmark asserts directly (max |Δp| ≤ 2e-26 across all four families). `reg_tidy_native_z()` therefore loses its rescale line and is byte-identical. The profile bounds never cross a store (`reg_fit_cacheable()` refuses `method = "profile"`), and `reg_digest_revive()` keeps only `g$fit`/`g$data`, so `rec$multiplier` was already inert there and stays as the recipe's own refit parameter.

**Tests.** The `multiplier` cache block asserted the **opposite** of the new behaviour ("a different scaling → a fresh fit") and is rewritten around `b$hits`, mirroring the existing "the estimand is NOT in the key" block — ⚠ asserting the hit *and* that the table re-reports *and* that the served table equals the cold one, since a hit serving a stale table would pass any one of those alone. New beside it: the p-value untouched by `k` (including a negative one), and the bracket the right way round. Elsewhere: a two-model Excel check export · a named list keeping its caption · every declared `(family × panel)` pair building · the `last`-skips-NA rule on both `tab()` axes and in `tab_reg()`, with `ref = "NA"` still opting in · every cut shape accepted under `na = "keep_for_predictors"` while a numeric one is still refused.

Full suite **FAIL 0 | PASS 9989**, and **no golden or snapshot moved** — nothing in the phase reaches a default rendered table.

**The build chain ran here** — `dev/generate_jamovi_js.R` → `node --check` → `md5sum` → `jmvtools::prepare()` → `md5sum` again (**all seven yaml and both generated `.h.R` unchanged**, no `yaml.dump()` rewrite) → `i18nUpdate("catalog")` → `i18nUpdate("fr")` → hand-fill → `msgfmt -c` → `install(home = "flatpak")`. Compiled `inst/i18n/fr.json`: **284 of 284 translated**, the one new msgid included.

**Still open: the maintainer's live pass.** The `ref2 =` header appearing over the off-axis group the moment an odds ratio is switched on, and the reference column headed with nothing when it holds nothing (including over Table variables); a `multiplier` pick on a multinomial, which should now feel like the estimand picks do; an Excel export from a **two-outcome** Regressions panel with *assumption checks plots* ticked; and an ordinal model's check grid, which now has five panels rather than four.
