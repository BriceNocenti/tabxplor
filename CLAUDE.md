...
"References, level merging and predictor scaling" main panel
- The UI is visually convoluted . I want to use the exact same "Reorder and merge levels" UI/box than `jmvtab`.
...
**Still open, deliberately:** the crude (`empirical = TRUE`) block is built before the specs and is **not cached**, so its refits are paid on every interaction; its key would be estimand-invariant too. Worth its own look if the Regressions panel still feels slow.
